...
//
// Build: g++ -std=c++20 db_tests.cpp db_row.cpp db_schema.cpp -I.. -lgtest
// -lgtest_main -pthread -o db_tests Or add to CMake test target.
//
...
public:
  ...
  i64 exec(const std::string &sql,
    ...
    if (create_pos != std::string::npos) {
      auto name_start =
          create_pos + std::string("CREATE TABLE IF NOT EXISTS ").size();
      ...
      if (name_end != std::string::npos) {
        ...
        if (first != std::string::npos)
          table_name = table_name.substr(first);
...
TEST_F(DbSchemaContentTest,
       SqlCreateSignatureRecords_LedgerStatusHasCorrectCheck) {
...
TEST_F(DbSchemaContentTest, SqlCreateSignatureRecords_CoreColumnsPresent) {
  ...
  for (const char *col :
       {"id", "created_at", "slot_id", "token_label", "key_id",
        ...
        "ledger_block_num", "ledger_status"}) {
...
TEST_F(DbSchemaBootstrapTest, Bootstrap_FreshDb_ExecsCreateStatements) {
  ...
  for (const auto &s : log) {
    if (s.find("signature_records") != std::string::npos)
      found_sig = true;
    if (s.find("db_meta") != std::string::npos)
      found_meta = true;
  }
  EXPECT_TRUE(found_sig)
      << "Expected CREATE TABLE ... signature_records in exec log";
...
TEST_F(DbSchemaVersionTest,
       CurrentVersion_MetaTableExistsButNoRow_ReturnsMinusOne) {
  ...
  void SetUp() override {
    for (const char *tbl :
         {"db_meta", "signature_records", "signature_verifications",
          "notification_subscribers", "notification_log",
          "key_rekor_registry"}) {
      conn_.exec("CREATE TABLE IF NOT EXISTS " + std::string(tbl) +
                 " (x TEXT);");
    }
...
TEST_F(DbSchemaMigrateTest, Migrate_AlreadyAtCurrentVersion_IsNoop) {
  ...
  for (const char *tbl :
       {"db_meta", "signature_records", "signature_verifications",
        "notification_subscribers", "notification_log"}) {
    current_conn.exec("CREATE TABLE IF NOT EXISTS " + std::string(tbl) +
                      " (x TEXT);");
  }

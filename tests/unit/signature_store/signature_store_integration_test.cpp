...
public:
    ...
    std::optional<vhsm::ledger::LedgerEntry> submit_record(const SignatureRecord& rec) override {
        ++submit_count;
        last_record = rec;
        return response;
    }
    ...
    vhsm::ledger::LedgerEntry response;  // what the worker will stamp on the record
...
protected:
    void SetUp() override {
        ...
        dir /= "vhsm_integration_" + std::to_string(
    ...
    void TearDown() override {
        conn_.reset();
        std::error_code ec;
        std::filesystem::remove_all(std::filesystem::path(db_path_).parent_path(), ec);
    }
    ...
    std::optional<std::string> insert_dummy_record(const std::string& key_fp) {
        return repo_->insert(
            1234567890, 0, "int-token", "key-" + key_fp, key_fp,
            "CKM_ECDSA_SHA256", "SHA256",
            "aabbccddeeff00112233445566778899", 4,
            "MEUCIQD...", "sess1", "alice", "caller-app");
    }
...
TEST_F(SignatureStoreIntegrationTest, SchemaIsIdempotentOnReopenAndVerifyHolds) {
    open_db();
...
TEST_F(SignatureStoreIntegrationTest, DispatchPersistsAndLedgerWorkerAnchorsIt) {
    open_db();
    ...
    std::unique_ptr<vhsm::ledger::LedgerWorker> worker;
    {
        ...
        worker = std::make_unique<vhsm::ledger::LedgerWorker>(
            ledger, bus,
            [db, tok, &cb_m, &cb_cv, &anchored_ids, &callback_count, &db_update_ok](
                const SignatureRecord& rec, const vhsm::ledger::LedgerEntry& entry) {
                {
    ...
    bool saw_committed = false;
    {
        ...
        for (const auto& e : bus.events) {
            if (e.type == vhsm::notification::NotificationEvent::EventType::LEDGER_COMMITTED &&
...
TEST_F(SignatureStoreIntegrationTest, LedgerWorkerWithoutCallbackPublishesOnly) {
    open_db();

// notification_dispatcher_test.cpp — Unit tests for the notification dispatcher:
// channel routing, severity gating, event-type filtering, retry/backoff for
// WARN/CRITICAL, and best-effort INFO delivery.
//
...
protected:
    void SetUp() override {
        ...
        dispatcher_ = std::make_unique<NotificationDispatcher>(
            *bus_, *repo_, NotificationDispatcher::RetryPolicy(
                /*max_retries=*/2,
                /*base_delay_ms=*/1,
                /*max_delay_ms=*/5,
                /*multiplier=*/2));
    ...
    void TearDown() override {
        if (dispatcher_) dispatcher_->drain_and_stop();
    }
    ...
    bool wait_for_delivery(FakeAdapter& adapter, int n, int timeout_ms = 3000) {
        ...
        while (std::chrono::steady_clock::now() < deadline) {
            if (adapter.delivered.load() >= n) return true;
...
TEST_F(NotificationDispatcherTest, SeverityGateBlocksLowerSeverityEvents) {
    ...
    ASSERT_TRUE(wait_for_delivery(email, 1));
    EXPECT_EQ(email.last_event.type, NotificationEvent::EventType::DB_WRITE_FAILED);
}

TEST_F(NotificationDispatcherTest, SeverityGateAllowsEqualOrHigher) {
    ASSERT_TRUE(repo_->add_subscriber("sub1", "Alice", "email",
                                      "alice@example.com", "WARN",
                                      std::nullopt, true));
...
TEST_F(NotificationDispatcherTest, EventPublishedBeforeSubscribersExistIsProcessedLater) {
    ...
    ASSERT_TRUE(wait_for_delivery(email, 1));
    EXPECT_EQ(email.last_event.type, NotificationEvent::EventType::VERIFY_COMPLETED);
}

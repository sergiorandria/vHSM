...
TEST(HsmObject, MoveSemantics) {
  ...
  // After move, obj1 should be in a default-constructed-like state
  // (ObjectType::OTHER, not sensitive, not extractable, empty id)
...
TEST(HsmObject, CopySemanticsThrowsForSensitive) {
  ...
  EXPECT_THROW({ HsmObject copy = sensitiveObj; }, std::runtime_error);

  HsmObject nonSensitiveObj(ObjectType::DATA, false, true);
  // Copying non-sensitive should be allowed
  EXPECT_NO_THROW({ HsmObject copy = nonSensitiveObj; });
}
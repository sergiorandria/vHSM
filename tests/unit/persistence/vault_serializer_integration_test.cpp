...
// helpers (snapshot_from_token -> Vault::create -> Vault -> restore_token_from_vault).
// This is the exact recovery path the admin RestoreToken RPC and C_Finalize
// autosave follow.
TEST(VaultSerializerIntegrationTest, LiveTokenSurvivesVaultRoundTrip) {
    ...
    ASSERT_EQ(source.initialize_so_pin(reinterpret_cast<const CK_CHAR*>("9999"), 4),
              CKR_OK);
    ASSERT_EQ(source.initialize_user_pin(reinterpret_cast<const CK_CHAR*>("1234"), 4),
              CKR_OK);
    // Pin out 2 failed attempts so the counter has non-default state to carry.
    ASSERT_EQ(source.login(CKU_USER, reinterpret_cast<const CK_CHAR*>("0000"), 4),
              CKR_PIN_INCORRECT);
    ASSERT_EQ(source.login(CKU_USER, reinterpret_cast<const CK_CHAR*>("0000"), 4),
              CKR_PIN_INCORRECT);
    ...
    source.restore_state(
        source.is_token_initialized(),
        source.is_user_pin_set(),
        source.is_so_pin_set(),
        source.is_user_login_required(),
        source.is_so_login_required(),
        source.max_pin_attempts(),
        source.user_pin_failed_attempts(),
        source.so_pin_failed_attempts(),
        source.is_user_pin_locked(),
        source.is_so_pin_locked(),
        kKek);
    ...
    auto vault = Vault::create(path, password,
...
TEST(VaultSerializerIntegrationTest, WrongPasswordDoesNotClobber) {
    ...
    auto vault = Vault::create(path, "right-password",
                               serialize_token_snapshot(snapshot_from_token(source)));
    ASSERT_TRUE(vault.is_valid());
    ...
    EXPECT_THROW(
        restore_token_from_vault(target, Vault(path, "wrong-password")),
        std::runtime_error);

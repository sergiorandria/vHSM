...
TEST(ThreadPoolTest, SubmitPropagatesExceptionThroughFuture) {
    ...
    auto f = pool.submit(kHigh, []() -> int { throw std::runtime_error("boom"); });
...
TEST(ThreadPoolTest, EnqueueRunsEveryTaskExactlyOnce) {
    ...
    for (int i = 0; i < kTasks; ++i)
        pool.enqueue(kHigh, [&ran] { ran.fetch_add(1, std::memory_order_relaxed); });
...
TEST(ThreadPoolTest, ConcurrentProducersLoseNoTasksWhenBacklogFits) {
    ...
    EXPECT_EQ(pool.enqueued_count(), static_cast<std::size_t>(kProducers * kPerProducer));
    EXPECT_EQ(pool.executed_count(), static_cast<std::size_t>(kProducers * kPerProducer));
    EXPECT_EQ(pool.dropped_count(), 0u);
    EXPECT_EQ(pool.queued_count(), 0u);
    EXPECT_EQ(pool.running_count(), 0u);
}
...
TEST(ThreadPoolTest, SingleCorePoolRunsHighAndLowTier) {
    ...
    for (int i = 0; i < 20; ++i) {
        pool.enqueue(kHigh, [&high] { high.fetch_add(1, std::memory_order_relaxed); });
...
TEST(ThreadPoolTest, BoundedQueuesDropWhenFull) {
    ...
    while (pool.running_count() != 1u && std::chrono::steady_clock::now() < deadline)

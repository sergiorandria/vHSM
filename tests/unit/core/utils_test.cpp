...
TEST(UuidV4, CorrectLength) { EXPECT_EQ(uuid_v4().size(), 36u); }

TEST(UuidV4, CorrectFormat) {
  // "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx"
  const std::regex re("^[0-9a-f]{8}-"
                      "[0-9a-f]{4}-"
                      "4[0-9a-f]{3}-"
                      "[89ab][0-9a-f]{3}-"
                      "[0-9a-f]{12}$");
...
TEST(Base64Encode, EmptyInput) { EXPECT_EQ(base64_encode({}), ""); }
...
TEST(Base64RoundTrip, ArbitraryBytes) {
  const std::vector<std::byte> original = {std::byte{0x00}, std::byte{0xff},
                                           std::byte{0x80}, std::byte{0x01},
                                           std::byte{0xfe}, std::byte{0x7f}};
...
TEST(HexEncode, EmptyInput) { EXPECT_EQ(hex_encode({}), ""); }

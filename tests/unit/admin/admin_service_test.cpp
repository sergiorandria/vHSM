...
protected:
    void SetUp() override {
        ...
        ASSERT_EQ(token_->initialize_user_pin(
                      reinterpret_cast<const CK_CHAR*>("1234"), 4), CKR_OK);
        ASSERT_EQ(token_->initialize_so_pin(
                      reinterpret_cast<const CK_CHAR*>("abcd"), 4), CKR_OK);
...
TEST_F(AdminLoginCoreTest, WrongPinDoesNotPublishEvent) {
    EXPECT_EQ(core_->admin_login(CKU_USER, "0000", "attacker"), CKR_PIN_INCORRECT);
...
protected:
    void SetUp() override {
        ...
        token_->restore_state(
            ...
            token_->is_user_pin_locked(), token_->is_so_pin_locked(),
            kKek);
...
TEST_F(TokenBackupCoreTest, BackupRestoreRoundTrip) {
    ...
    ASSERT_EQ(token_->login(CKU_USER,
                            reinterpret_cast<const CK_CHAR*>("0000"), 4),
              CKR_PIN_INCORRECT);
    ASSERT_EQ(token_->login(CKU_USER,
                            reinterpret_cast<const CK_CHAR*>("0000"), 4),
              CKR_PIN_INCORRECT);
...
TEST_F(TokenBackupCoreTest, BackupMissingFileRestoreThrows) {
    ...
    EXPECT_THROW(restore_core.restore((dir_ / "does-not-exist.vault").string(),
                                      "pw"),
                 std::runtime_error);
    EXPECT_EQ(fresh.get_kek(), kek_before);
}
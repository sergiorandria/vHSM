...
TEST(CTR_DRBG_AES256, ConstructorValidEntropy) {
  ...
  ASSERT_NO_THROW({ CTR_DRBG_AES256 rng(entropy); });
}
...
TEST(CTR_DRBG_AES256, ReseedThreshold) {
  // This test would take too long if we actually hit the reseed threshold
  // (100000) Instead, we'll test the logic by manually setting a low threshold
  // for testing But since RESEED_INTERVAL is private, we can't easily test this
  // without modification For now, we'll just verify the function exists and
  // doesn't throw for normal use
...
TEST(CTR_DRBG_AES256, ForwardSecurity) {
  // Test that update() is called with empty vector after generate (forward
  // security) This is harder to test directly since update() is private We can
  // test that successive calls produce different sequences

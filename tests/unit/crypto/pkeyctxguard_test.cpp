#include "../../../src/crypto/pkey_ctx_guard.h"
...
TEST(PkeyCtxGuardTest, ConstructorAndDestruction) {
  ...
  // For testing purposes, we'll test with nullptr which should be handled
  // safely
...
TEST(PkeyCtxGuardTest, ValidContextHandling) {
  ...
  // Note: This might fail if no engine is available, but we'll test the guard
  // behavior

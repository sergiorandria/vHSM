#include "../../../src/crypto/md_ctx_guard.h"
...
TEST(MdCtxGuardTest, ConstructorAndDestruction) {
  ...
  // We can't easily test that it's freed without causing UB by using freed
  // memory But we can verify the guard doesn't leak by construction/destruction
}

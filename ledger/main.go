...
// CreateThesisRecord - Enregistre une nouvelle these avec ID auto-généré
func (s *SmartContract) CreateThesisRecord(ctx contractapi.TransactionContextInterface, docHash string, grade string) (string, error) {
	idTX := ctx.GetStub().GetTxID()
	thesisID := generateShortID(idTX)

...
// with 12 positional args and returned `optional<string>` via raw SQL. New
// domain code should talk to `ISignatureStore::store(SignatureRecord)` with a
// single aggregate, so the DB specifics (column order, `ledger_*` defaults) stay
// hidden. This adapter translates the port call into the existing repository
...
public:
  ...
  std::optional<std::string> store(const SignatureRecord& rec) override {
    return repo_.insert(rec.created_at, rec.slot_id, rec.token_label,
                        ...
                        rec.payload_size, rec.signature_b64,
                        rec.session_handle, rec.user_label, rec.app_context);
  }
  ...
  std::optional<SignatureRecord> load(const std::string& id) const override {
    ...
    if (row->size() > 0 && (*row)[0]) rec.record_id = *(*row)[0];
    return rec;
  }

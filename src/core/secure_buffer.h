...
// WHY SecureBuffer locks memory with mlock(): Sensitive data (keys,
// credentials, PINs) must never be paged to disk. mlock() prevents the OS from
// evicting locked pages to swap. If the HSM is compromised and the attacker
// reads /swapfile, secrets won't be found there. This is defense-in-depth: even
// a privileged process (with code-execution-adjacent access) can't recover keys
// from disk if we lock them in RAM.
//
// WHY memory is zeroed before freeing: mlock() only prevents swapping; it
// doesn't prevent memory dumps or physical attacks. Zeroing (using secure_zero,
// not memset) overwrites the bytes with zeros using methods the compiler can't
// optimize away (volatile writes). After free(), the allocator might reuse the
// memory for non-sensitive data; we don't want garbage collecting to reveal old
// key bytes.
//
// WHY template parameter explicitly rejected: Templating SecureBuffer would
// generate separate code for SecureBuffer<u8>, SecureBuffer<u16>, etc. This
// bloats the binary and complicates auditing (code paths multiply). The
// byte-oriented design (u8-only) keeps it simple, auditable, and focused on the
// security contract.
class SecureBuffer {
public:
  // WHY explicit constructor with default size=1: explicit prevents accidental
  // implicit conversions (size_t → SecureBuffer). Default size=1 allows
  // (SecureBuffer sb;) without forcing size specification. The size is in
  // elements (bytes for u8), not bytes directly.
  explicit SecureBuffer(std::size_t element_count = 1);

  ~SecureBuffer() noexcept;

  // WHY non-copyable: Copying would create a second buffer with the same
  // sensitive data. Doubling the attack surface is unacceptable. Callers must
  // move (transfer ownership) or use references. This constraint enforces
  // single ownership of sensitive data.
  SecureBuffer(const SecureBuffer &) = delete;
  SecureBuffer &operator=(const SecureBuffer &) = delete;

  // WHY moves are noexcept: Containers (std::vector, std::unique_ptr) require
  // move to be noexcept for exception safety. If move threw, the vector would
  // be in an invalid state. noexcept guarantees atomicity: either the move
  // succeeds, or the source is unchanged.
  ...
  // bounds (throw if offset+len > size), preventing buffer overruns. Both
  // methods throw std::out_of_range on bounds violation (fail-closed). Write
  // `len` bytes from `src` into the buffer at `offset`. Throws
  // std::out_of_range if len > size() - offset.
  ...
  // WHY equals() instead of operator==: Two SecureBuffers are equal if their
  // contents match. But operator== is usually used for identity (this buffer ==
  ...
  // std::map). They delegate to equals() for semantic clarity.
  bool operator==(const SecureBuffer &other) const noexcept;
  bool operator!=(const SecureBuffer &other) const noexcept;

  // WHY separate wipe() method: Explicit zeroing is sometimes needed before
  // destroying the buffer (e.g., logging reveals we're about to destruct a
  // key). The wipe() method lets callers zero the contents without
  // deallocating. The destructor calls wipe() automatically, so double-wipe is
...
// This is a separate class to avoid template bloat in the main SecureBuffer
// class.
template <typename T = u8> class SecureBufferT {
public:

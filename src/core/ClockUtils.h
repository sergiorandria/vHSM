...
struct ClockUtils {
  ...
  static std::optional<HsmTimePoint> from_iso8601(const std::string &s) {
    ...
    if (s.size() == 24) {
      ...
      if (std::sscanf(s.c_str(), "%4d-%2d-%2dT%2d:%2d:%2dZ", &utc_tm.tm_year,
                      &utc_tm.tm_mon, &utc_tm.tm_mday, &utc_tm.tm_hour,
                      &utc_tm.tm_min, &utc_tm.tm_sec) != 6)
        return std::nullopt;
    } else {
      return std::nullopt;
    }
    ...
    if (utc_tm.tm_mon < 0 || utc_tm.tm_mon > 11)
      return std::nullopt;
    if (utc_tm.tm_mday < 1 || utc_tm.tm_mday > 31)
      return std::nullopt;
    if (utc_tm.tm_hour > 23)
      return std::nullopt;
    if (utc_tm.tm_min > 59)
      return std::nullopt;
    if (utc_tm.tm_sec > 60)
      return std::nullopt; // 60 = leap second
    if (ms < 0 || ms > 999)
      return std::nullopt;

...
std::vector<u8> serialize_token_snapshot(const TokenSnapshot &snap) {
  VHSM_CHECK_MSG(snap.label.size() <= 255,
                 "serialize_token_snapshot: label too long");
  VHSM_CHECK_MSG(snap.id.size() <= 255,
                 "serialize_token_snapshot: id too long");
  ...
  u8 flags = 0;
  if (snap.token_initialized)
    flags |= KTokenInit;
  if (snap.user_pin_set)
    flags |= KUserPinSet;
  if (snap.so_pin_set)
    flags |= KSoPinSet;
  if (snap.user_login_required)
    flags |= KUserLoginReq;
  if (snap.so_login_required)
    flags |= KSoLoginReq;
  if (snap.user_pin_locked)
    flags |= KUserPinLocked;
  if (snap.so_pin_locked)
    flags |= KSoPinLocked;
  out.push_back(flags);
...
TokenSnapshot deserialize_token_snapshot(const std::vector<u8> &data) {
  VHSM_CHECK_MSG(data.size() >= 8 + 2,
                 "deserialize_token_snapshot: data too short");
  ...
  VHSM_CHECK_MSG(version == KVersion,
                 "deserialize_token_snapshot: unsupported version");

  std::size_t off = 8;

  const auto read_string = [&]() -> std::string {
    VHSM_CHECK_MSG(off < data.size(),
                   "deserialize_token_snapshot: truncated string length");
    const std::size_t len = data[off++];
    VHSM_CHECK_MSG(len <= data.size() - off,
                   "deserialize_token_snapshot: truncated string");
  ...
  snap.max_session_count = static_cast<CK_ULONG>(get_le64(p + off));
  off += 8;
  snap.session_count = static_cast<CK_ULONG>(get_le64(p + off));
  off += 8;
  snap.max_rw_session_count = static_cast<CK_ULONG>(get_le64(p + off));
  off += 8;
  snap.rw_session_count = static_cast<CK_ULONG>(get_le64(p + off));
  off += 8;
  ...
  snap.max_failed_attempts = get_le32(p + off);
  off += 4;
  snap.user_failed_attempts = get_le32(p + off);
  off += 4;
  snap.so_failed_attempts = get_le32(p + off);
  off += 4;

  const std::uint64_t kek_len = get_le64(p + off);
  off += 8;
  VHSM_CHECK_MSG(kek_len <= data.size() - off,
                 "deserialize_token_snapshot: truncated KEK");
...
void restore_token_from_snapshot(vhsm::keystore::Token &token,
  ...
  token.restore_state(snap.token_initialized, snap.user_pin_set,
                      snap.so_pin_set, snap.user_login_required,
                      snap.so_login_required, snap.max_failed_attempts,
                      snap.user_failed_attempts, snap.so_failed_attempts,
                      snap.user_pin_locked, snap.so_pin_locked, snap.kek);
}

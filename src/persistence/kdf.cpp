...
std::vector<u8> vhsm::persistence::derive_vault_key(const std::string &password,
  ...
  const int rc = PKCS5_PBKDF2_HMAC(
      ...
      static_cast<int>(out_len), out.data());
...
std::vector<u8> vhsm::persistence::hkdf_sha256(const std::vector<u8> &ikm,
  ...
  const bool ok = HMAC(EVP_sha256(), salt_effective.data(),
                       static_cast<int>(salt_effective.size()), ikm.data(),
                       ikm.size(), prk.data(), &prk_len) != nullptr;
  ...
  for (std::uint8_t i = 1; remaining > 0; ++i, --remaining) {
    ...
    const bool ok2 =
        HMAC(EVP_sha256(), prk.data(), static_cast<int>(prk.size()),
             input.data(), input.size(), block.data(), &block_len) != nullptr;
    VHSM_CHECK_MSG(ok2, "hkdf_sha256: HMAC(Expand) failed");
    VHSM_CHECK_MSG(block_len == kHashLen,
                   "hkdf_sha256: unexpected block length");
...
vhsm::persistence::derive_db_hmac_key(const std::vector<u8> &vault_kek) {
  static const std::vector<u8> kInfo = {'v', 'H', 'S', 'M', '-', 'd',
                                        'b', '-', 'h', 'm', 'a', 'c'};

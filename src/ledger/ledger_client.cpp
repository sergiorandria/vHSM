...
LedgerClient::LedgerClient(const std::string& gateway_endpoint,
    ...
    if (cert_path.empty() || key_path.empty()) {
        throw std::runtime_error("cert_path and key_path are required for mTLS + Fabric identity signing");
    }
    ...
    std::shared_ptr<fabric::grpc::GrpcConnection> connection =
        fabric::grpc::GrpcConnection::connect(gateway_endpoint, tls, defaultChannelOptions());
    if (!connection) {
        throw std::runtime_error("Failed to create gRPC connection to " + gateway_endpoint);
    }
    ...
    if (!identity.isValid()) {
        throw std::runtime_error("Invalid Fabric identity: certificate or key is empty");
    }
...
std::optional<LedgerEntry> LedgerClient::get_record(const std::string& record_id) {
    ...
    try {
        ...
        std::cerr << "Failed to parse chaincode response: " << e.what() << std::endl;
        return std::nullopt;
    }

...
namespace vhsm::threadpool {
    ...
    public:
        ...
        std::size_t enqueue_batch(const CapabilityToken& token, Iterator begin, Iterator end);
        ...
        std::size_t queued_count() const noexcept { return counters_.queued.load(std::memory_order_acquire); }
        std::size_t running_count() const noexcept { return counters_.running.load(std::memory_order_acquire); }
        std::size_t enqueued_count() const noexcept { return counters_.enqueued.load(std::memory_order_acquire); }
        std::size_t executed_count() const noexcept { return counters_.executed.load(std::memory_order_acquire); }
        std::size_t dropped_count() const noexcept { return counters_.dropped.load(std::memory_order_acquire); }
        std::size_t stolen_count() const noexcept { return counters_.stolen.load(std::memory_order_acquire); }
        ...
        -> std::future<std::invoke_result_t<Callable, Args...>>
    {
        ...
        auto task = std::make_shared<std::packaged_task<return_type()>>(
            [fn = std::forward<Callable>(fn), ...args = std::forward<Args>(args)]() mutable -> return_type {
        ...
        if (!accepted)
            throw std::runtime_error("ThreadPool: queue capacity exceeded, task rejected.");
        work_cv_.notify_one();
        return result;
    }

    inline auto ThreadPool::enqueue(const CapabilityToken& token, VoidCallable auto task) -> bool
    {
        validate_token(token);
    ...
    template <typename Iterator>
    std::size_t ThreadPool::enqueue_batch(const CapabilityToken& token, Iterator begin, Iterator end)
    {
        validate_token(token);

 ...
 *
 * WHY AES-256-GCM (Galois/Counter Mode): GCM combines encryption (CTR mode) with
 * authentication (GHASH). This prevents tampering: if an attacker modifies the
 * ciphertext, decryption throws (tag verification fails). Unlike ECB or CBC
...
{
// WHY separate struct: Bundling the three components (ciphertext, nonce, tag) prevents
// them from being accidentally mismatched during transport. A caller can't accidentally
// use the tag from one operation with the ciphertext from another.
struct AESGCMResult
{
...
{
public:
    // WHY static methods: No state is needed. Encrypt/decrypt are pure cryptographic
    // operations. Static methods signal "these are utilities, not objects that hold state".
    // Thread-safe: each call uses separate OpenSSL contexts.
    static AESGCMResult encrypt(const std::vector<u8>& key, const std::vector<u8>& plaintext);
    
    // WHY decrypt returns std::vector (not optional): On success, it returns the plaintext.
    // On failure (tag mismatch), it throws runtime_error. This is fail-closed: no return
    // of invalid/corrupted plaintext. The caller either gets valid data or an exception.

 ...
 *   should ensure key lengths are correct and handle exceptions on auth
 * failure.
 */
#include "aes_gcm.h"
...
AESGCMResult AESGCM::encrypt(const std::vector<u8> &key,
  ...
  VHSM_CHECK_MSG(key.size() == 32,
                 "AESGCM::encrypt: key must be 32 bytes (AES-256)");
  ...
  VHSM_CHECK(EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr,
                                nullptr) == 1);
  VHSM_CHECK(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN,
                                 static_cast<int>(result.nonce.size()),
                                 nullptr) == 1);
  VHSM_CHECK(EVP_EncryptInit_ex(ctx, nullptr, nullptr, key.data(),
                                result.nonce.data()) == 1);

  result.ciphertext.resize(plaintext.size());

  VHSM_CHECK(EVP_EncryptUpdate(ctx, result.ciphertext.data(), &len,
                               plaintext.data(),
                               static_cast<int>(plaintext.size())) == 1);
  VHSM_CHECK(EVP_EncryptFinal_ex(ctx, result.ciphertext.data() + len,
                                 &final_len) == 1);
  ...
  VHSM_CHECK(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16,
                                 result.tag.data()) == 1);
...
std::vector<u8> AESGCM::decrypt(const std::vector<u8> &key,
  ...
  VHSM_CHECK_MSG(key.size() == 32,
                 "AESGCM::decrypt: key must be 32 bytes (AES-256)");
  VHSM_CHECK_MSG(data.nonce.size() >= 1 && data.nonce.size() <= 16,
                 "AESGCM::decrypt: nonce must be 1-16 bytes");
  VHSM_CHECK_MSG(data.tag.size() == 16,
                 "AESGCM::decrypt: tag must be 16 bytes");
  ...
  VHSM_CHECK(EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr,
                                nullptr) == 1);
  VHSM_CHECK(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN,
                                 static_cast<int>(data.nonce.size()),
                                 nullptr) == 1);
  VHSM_CHECK(EVP_DecryptInit_ex(ctx, nullptr, nullptr, key.data(),
                                data.nonce.data()) == 1);

  std::vector<u8> plaintext(data.ciphertext.size());

  VHSM_CHECK(EVP_DecryptUpdate(ctx, plaintext.data(), &len,

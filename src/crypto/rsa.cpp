...
RSAKeyPair RSAUtil::generate_key(int bits) {
  ...
  VHSM_CHECK_MSG(ctx != nullptr,
                 "RSAUtil::generate_key: EVP_PKEY_CTX_new_id failed");
...
std::vector<uint8_t> RSAUtil::sign(EVP_PKEY *key,
...
// Verify a signature over `data` using the provided EVP_PKEY RSA key and
// SHA-256. Returns true if verification succeeds, false on signature mismatch.
// Throws on OpenSSL internal error (rc < 0).
bool RSAUtil::verify(EVP_PKEY *key, const std::vector<uint8_t> &data,
  ...
  VHSM_CHECK_MSG(rc >= 0,
                 "RSAUtil::verify: EVP_DigestVerify returned an error");

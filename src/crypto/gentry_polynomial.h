...
public:
    ...
    explicit Polynomial(std::vector<Coeff> coeffs) : coeffs_(std::move(coeffs)) { trim(); }

    /// Constant polynomial.
    explicit Polynomial(Coeff c) : coeffs_({c}) { trim(); }

    /// Zero polynomial of the given degree.
    static Polynomial zero(std::size_t deg = 0) { return Polynomial(std::vector<Coeff>(deg + 1, 0LL)); }
    ...
    Coeff operator[](std::size_t i) const { return i < coeffs_.size() ? coeffs_[i] : 0LL; }
    Coeff& operator[](std::size_t i)
    {
        if (i >= coeffs_.size())
        {
            coeffs_.resize(i + 1, 0LL);
        }
        return coeffs_[i];
    }
     ...
     */
    Polynomial ring_mul(const Polynomial& o, const Polynomial& f) const { return (*this * o).reduce_mod(f); }

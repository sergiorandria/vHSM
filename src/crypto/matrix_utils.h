...
inline Matrix invert(const Matrix& A) {
    ...
    for (int col = 0; col < n; ++col) {
        ...
        for (int j = col; j < 2 * n; ++j) aug[col][j] /= diag;
...
inline Matrix invert_imatrix(const IMatrix& M) {
    return invert(to_double(M));
}
...
inline double determinant(const Matrix& A) {
    ...
    for (int col = 0; col < n; ++col) {
        ...
        for (int row = col + 1; row < n; ++row)
            if (std::abs(aug[row][col]) > std::abs(aug[pivot][col]))
                pivot = row;
        if (pivot != col) { std::swap(aug[col], aug[pivot]); det = -det; }

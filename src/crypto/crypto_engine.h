 ...
 *
 * WHY CryptoEngine is a single orchestration point: All signing in vHSM flows through
 * this class. This centralizes policy:
 ...
 * By funneling all signatures here, we prevent bypasses and ensure consistent behavior.
 *
 * WHY fail-closed on mechanism mismatch: vHSM must never silently produce a wrong-algorithm
 * signature. If a caller requests RSA-PKCS but passes an EC key, we don't fail (transaction
 * aborts). Instead, we sign with the key's native algorithm (ECDSA) and record the actual
 * mechanism in SignResult. This ensures:
...
public:
     ...
     *                              Caller retains ownership and lifetime responsibility.
     ...
     *
     * WHY requested_mechanism is a string hint, not enforced: PKCS#11 applications pass
     * mechanism IDs (CKM_*). We accept a string for clarity in logs, but treat it as
     * a hint. If the hint is incompatible with the key type, we ignore it and sign with
     * the key's native algorithm. This is fail-closed: the signature always succeeds,
     * but the ledger records what was actually used. Callers who care can inspect
     * SignResult::mechanism_str to detect mismatches.
     *
     * @return SignResult carrying the signature, the digest, and the mechanism
     *         that was actually applied (for auditability). If the requested mechanism
     *         doesn't match the key type, mechanism_str contains the actual mechanism used.
     */
    static SignResult sign(EVP_PKEY* key,

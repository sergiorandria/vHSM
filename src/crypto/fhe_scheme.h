...
struct Ciphertext
{
    ...
    explicit Ciphertext(int n = 0, int lvl = 0) : data(n, 0LL), level(lvl) {}

    explicit Ciphertext(std::vector<long long> d, int lvl = 0) : data(std::move(d)), level(lvl) {}
...
public:
     ...
     */
    Plaintext decrypt(const SecretKey& sk, const PublicKey& pk, const Ciphertext& ct);
     ...
     */
    Ciphertext eval_add(const PublicKey& pk, const Ciphertext& c1, const Ciphertext& c2);

    /**
     * Mult(pk, ψ₁, ψ₂) = ψ₁ × ψ₂ mod B^pk_J
     * Multiplication in R = Z[x]/f(x), then reduced mod J.
     */
    Ciphertext eval_mul(const PublicKey& pk, const Ciphertext& c1, const Ciphertext& c2);

    /**
     * NAND gate:  NAND(a, b)  = 1 - a·b   (for {0,1} plaintexts).
     * Used as the universal gate for bootstrapping (§2).
     */
    Ciphertext eval_nand(const PublicKey& pk, const Ciphertext& c1, const Ciphertext& c2);
...
protected:
     ...
     */
    std::vector<long long> samp(const std::vector<long long>& pi_vec, const PublicKey& pk);

    // Reduce mod public key basis
    std::vector<long long> reduce_mod_pk(const std::vector<long long>& v, const PublicKey& pk);

    // Polynomial ring multiplication of ciphertext vectors
    std::vector<long long> ring_mul_vecs(const std::vector<long long>& a, const std::vector<long long>& b,
                                         const PublicKey& pk);
};
...
{
public:
    explicit SchemeE2(FHEParams params, u64 seed = 42) : SchemeE1(std::move(params), seed) {}
     ...
     */
    Plaintext decrypt_tweak1(const SecretKey& sk, const PublicKey& pk, const Ciphertext& ct);
};
...
{
public:
    explicit SchemeE3(FHEParams params, u64 seed = 42) : SchemeE2(std::move(params), seed) {}
     ...
     */
    Plaintext decrypt_e3(const SecretKey& sk, const PublicKey& pk, const Ciphertext& ct);
     ...
     */
    void split_key(const SecretKey& sk, const PublicKey& pk, std::vector<std::vector<long long>>& tau_out,
...
private:
    ...
    std::vector<std::vector<long long>> expand_ct(const PublicKey& pk, const std::vector<long long>& psi_star);
};
 ...
 */
Ciphertext recrypt(SchemeE3& scheme, const PublicKey& pk2, const SecretKey& sk1, const PublicKey& pk1,
                   const Ciphertext& psi1);

//  Circuit evaluation helpers
/// Evaluate a boolean function given as a truth-table (2^arity entries).
Ciphertext eval_boolean(SchemeE1& scheme, const PublicKey& pk, const std::vector<Ciphertext>& inputs,
                        const std::vector<int>& truth_table);

/// Evaluate a NAND tree homomorphically.
Ciphertext eval_nand_tree(SchemeE1& scheme, const PublicKey& pk, const Ciphertext& a, const Ciphertext& b);

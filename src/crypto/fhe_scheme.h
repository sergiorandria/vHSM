 ...
 *   Layer 2  –  SchemeE2  (§4):  E₁ with the two tweaks; lowers decryption
 * depth. Layer 3  –  SchemeE3  (§5):  E₂ + "squashing" transformation;
 * bootstrappable.
 *
...
struct PublicKey {
  ...
  std::vector<std::vector<long long>>
      tau; ///< γ_set random vectors in J^{-1} mod B_I.
};
...
public:
   ...
   */
  Ciphertext eval_add(const PublicKey &pk, const Ciphertext &c1,
                      const Ciphertext &c2);
   ...
   */
  Ciphertext eval_mul(const PublicKey &pk, const Ciphertext &c1,
                      const Ciphertext &c2);
   ...
   */
  Ciphertext eval_nand(const PublicKey &pk, const Ciphertext &c1,
                       const Ciphertext &c2);
 ...
 */
Ciphertext recrypt(SchemeE3 &scheme, const PublicKey &pk2, const SecretKey &sk1,
                   const PublicKey &pk1, const Ciphertext &psi1);

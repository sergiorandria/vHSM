...
ECCKeyPair ECC::generate_key(Curve curve) {
  ...
  VHSM_CHECK_MSG(ctx != nullptr,
                 "ECC::generate_key: EVP_PKEY_CTX_new_id failed");
  PkeyCtxGuard guard(ctx);

  VHSM_CHECK(EVP_PKEY_keygen_init(ctx) == 1);
  VHSM_CHECK(EVP_PKEY_CTX_set_ec_paramgen_curve_nid(ctx, curve_to_nid(curve)) ==
             1);
...
std::vector<u8> ECC::sign(EVP_PKEY *key, const std::vector<u8> &data) {
  ...
  size_t siglen = 0;
  VHSM_CHECK(EVP_DigestSign(ctx, nullptr, &siglen, data.data(), data.size()) ==
             1);
...
bool ECC::verify(EVP_PKEY *key, const std::vector<u8> &data,
  ...
  VHSM_CHECK(EVP_DigestVerifyInit(ctx, nullptr, EVP_sha256(), nullptr, key) ==
             1);

  const int rc = EVP_DigestVerify(ctx, signature.data(), signature.size(),
                                  data.data(), data.size());
...
std::vector<u8> ECC::derive_shared_secret(EVP_PKEY *private_key,
                                          EVP_PKEY *peer_public_key) {
  VHSM_CHECK_MSG(private_key != nullptr,
                 "ECC::derive_shared_secret: private_key is null");
  VHSM_CHECK_MSG(peer_public_key != nullptr,
                 "ECC::derive_shared_secret: peer_public_key is null");

  EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new(private_key, nullptr);
  VHSM_CHECK_MSG(ctx != nullptr,
                 "ECC::derive_shared_secret: EVP_PKEY_CTX_new failed");
  PkeyCtxGuard guard(ctx);

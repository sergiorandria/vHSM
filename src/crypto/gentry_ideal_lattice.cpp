...
IMatrix rotation_basis(const Polynomial &v, const Polynomial &f) {
  int n = f.degree();
  if (n <= 0) {
    throw std::invalid_argument("rotation_basis: f must have positive degree");
  }
...
std::vector<long long> vec_mod_basis(const std::vector<long long> &t,
                                     const IMatrix &B, const Matrix &B_inv) {
...
double compute_rDec(const Matrix &B_sk_inv) {
  ...
  if (max_col_norm == 0.0) {
    throw std::runtime_error("compute_rDec: degenerate basis");
  }
  return 1.0 / (2.0 * max_col_norm);
}

//  Samplers
std::vector<long long> sample_gaussian(int n, double sigma,
                                       std::mt19937_64 &rng) {
...
std::vector<long long> sample_uniform(int n, long long bound,
                                      std::mt19937_64 &rng) {

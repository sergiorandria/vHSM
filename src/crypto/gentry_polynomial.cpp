...
Polynomial Polynomial::reduce_mod(const Polynomial& f) const {
    if (f.is_zero()) throw std::invalid_argument("reduce_mod: modulus is zero");
    int fn = f.degree();
    if (fn <= 0) throw std::invalid_argument("reduce_mod: modulus must have degree >= 1");

    // Leading coefficient of f must be 1 (monic).
    if (f[fn] != 1) throw std::invalid_argument("reduce_mod: modulus must be monic");
...
double Polynomial::l2_norm() const {
    double s = 0.0;
    for (auto c : coeffs_) s += (double)c * (double)c;
    return std::sqrt(s);
}

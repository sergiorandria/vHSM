...
// WHY: Abstract base class for all keystore objects. We need a unified interface
// because PKCS#11 requires handling diverse object types (keys, certs, data) with
// a single handle-based API. By centralizing type metadata, copyability constraints,
// and attribute storage here, derived classes (PrivateKey, Certificate, etc.) 
// inherit consistent semantics without reimplementing the same patterns.
//
...
// the attack surface and memory footprint. The copy constructor explicitly checks
// sensitive_ and throws. Non-sensitive objects (certificates, public keys) can be
// copied for transport across module boundaries.
//
...
// cache or memory dumps. SecureBuffer zeroes memory before deallocation, protecting
...
public:
    ...
    // WHY separate getters for each boolean flag: PKCS#11 spec requires individual
    // boolean queries (C_GetObjectAttribute with CKA_SENSITIVE, CKA_EXTRACTABLE, etc.)
    // by callers. Exporting a struct would force callers to request all flags or 
    // rebuild from a bitmask. Individual getters align with the standard API.
    ...
    // they travel with the object in serialization. v_AttributeStore_M1 is declared
    // friend to enforce read-only access for most code (only factory sets attrs).
    //
    // WHY findAttribute returns const pointer (not optional/variant): Pointer allows
    // zero-copy access to the vector. null = not found. Avoids copies or allocation
    // for the common case (attribute not present).
    //
...
protected:
    ...
    // wipe(), we ensure each level zeros its data in the correct order, preventing
    // key fragments from surviving on the stack or registers.

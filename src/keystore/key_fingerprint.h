...
// WHY KeyFingerprint static utility class: Key fingerprinting is a pure function
// (same key → same fingerprint). No state needed. Making it a static utility class
// (no instances) makes this clear and prevents accidental instantiation.
//
// WHY fingerprints from keys/SPKI: Different representations of the same key must
// produce the same fingerprint. We support:
...
public:   
     ...
     *
     * WHY from_SPKI takes raw bytes: SPKI is the standard ASN.1 encoding from certificates
     * and CSRs. Accepting raw DER avoids needing an ASN.1 parser at this layer.
     * The caller (certificate import) provides the bytes; we hash them.
     */
    ...
    // Overloads let us extract the canonical public key bytes for each type, then hash.
    // This ensures keys imported as SPKI match keys imported as objects.

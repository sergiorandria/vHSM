...
Token::Token(const std::string &label, const std::string &id)
    : v_clock_(), v_core_(label, id, v_clock_) {}
...
const std::string &Token::get_id() const noexcept { return v_core_.v_get_id(); }
...
Token::get_object(CK_OBJECT_HANDLE handle) const {
  return v_core_.v_get_object(handle);
}
...
std::vector<std::uint8_t> Token::get_kek() const { return v_core_.v_get_kek(); }
...
CK_RV Token::set_user_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
...
CK_RV Token::set_so_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
                        const CK_CHAR *newPin, CK_ULONG newLen) {
  if ((oldPin == nullptr && oldLen > 0) || (newPin == nullptr && newLen > 0)) {
    return CKR_ARGUMENTS_BAD;
  }
...
CK_RV Token::change_user_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
                             const CK_CHAR *newPin, CK_ULONG newLen) {
  if ((oldPin == nullptr && oldLen > 0) || (newPin == nullptr && newLen > 0)) {
    return CKR_ARGUMENTS_BAD;
  }
...
CK_RV Token::change_so_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
                           const CK_CHAR *newPin, CK_ULONG newLen) {
  if ((oldPin == nullptr && oldLen > 0) || (newPin == nullptr && newLen > 0)) {
    return CKR_ARGUMENTS_BAD;
  }
...
void Token::increment_session_count() { v_core_.v_increment_session_count(); }

void Token::decrement_session_count() { v_core_.v_decrement_session_count(); }
...
void Token::restore_state(CK_BBOOL token_initialized, CK_BBOOL user_pin_set,
  ...
  v_core_.v_restore_state(
      token_initialized, user_pin_set, so_pin_set, user_login_required,
      so_login_required, max_failed_attempts, user_failed_attempts,
      so_failed_attempts, user_pin_locked, so_pin_locked, kek);
}

...
v_TokenCore_M1::v_TokenCore_M1(const std::string& label,
                               const std::string& id,
                               const IHsmClock& clock)
    : v_label_(label)
    , v_id_(id)
    , v_session_count_(0)
    , v_rw_session_count_(0)
    , v_token_initialized_(CK_FALSE)
    , v_user_pin_set_(CK_FALSE)
    , v_so_pin_set_(CK_FALSE)
    , v_user_login_required_(CK_TRUE)
    , v_so_login_required_(CK_TRUE)
    , v_clock_(clock)
...
const std::string& v_TokenCore_M1::v_get_id() const noexcept {
    return v_id_;
}
...
std::vector<std::uint8_t> v_TokenCore_M1::v_get_kek() const {
    auto result = v_object_store_.v_find_object_if([&](HsmObject* obj) {
        ...
        if (rv != CKR_OK) return false;
        if (class_value.size() != sizeof(CK_ULONG)) return false;

        CK_ULONG obj_class = 0;
        std::memcpy(&obj_class, class_value.data(), sizeof(CK_ULONG));
        if (obj_class != CKO_SECRET_KEY) return false;
        return true;
    });
...
CK_RV v_TokenCore_M1::v_initialize_user_pin(const CK_CHAR* pin, CK_ULONG pinLen) {
    v_touch_pin_op();
...
CK_RV v_TokenCore_M1::v_set_user_pin(const CK_CHAR* oldPin, CK_ULONG oldLen, const CK_CHAR* newPin, CK_ULONG newLen) {
    v_touch_pin_op();
    std::lock_guard<std::mutex> lock(v_user_pin_mutex_);
...
CK_RV v_TokenCore_M1::v_set_so_pin(const CK_CHAR* oldPin, CK_ULONG oldLen, const CK_CHAR* newPin, CK_ULONG newLen) {
    v_touch_pin_op();
    std::lock_guard<std::mutex> lock(v_so_pin_mutex_);
...
CK_RV v_TokenCore_M1::v_change_so_pin(const CK_CHAR* oldPin, CK_ULONG oldLen, const CK_CHAR* newPin, CK_ULONG newLen) {
    return v_set_so_pin(oldPin, oldLen, newPin, newLen);
}

CK_RV v_TokenCore_M1::v_login(CK_USER_TYPE userType, const CK_CHAR* pin, CK_ULONG pinLen) {

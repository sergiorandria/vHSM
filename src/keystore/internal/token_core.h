...
public:
  ...
  v_TokenCore_M1(const std::string &label, const std::string &id,
  ...
  std::pair<CK_OBJECT_HANDLE, T *> v_create_object(Args &&...args) {
    ...
    return v_object_store_.template v_create_object<T>(
        std::forward<Args>(args)...);
  }

  template <typename T>
  T *v_find_object_by_label_and_id(const std::string &label,
                                   const std::string &id) {
    auto result = v_object_store_.v_find_object_if([&](HsmObject *obj) {
      ...
      rv =
          attr_store.v_get_attribute(CKA_LABEL, label_value.data(), &label_len);
      if (rv != CKR_OK)
        return false;
  ...
  CK_RV v_set_user_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
                       const CK_CHAR *newPin, CK_ULONG newLen);
  CK_RV v_set_so_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
                     const CK_CHAR *newPin, CK_ULONG newLen);
  CK_RV v_verify_user_pin(const CK_CHAR *pin, CK_ULONG pinLen);
  CK_RV v_verify_so_pin(const CK_CHAR *pin, CK_ULONG pinLen);
  CK_RV v_change_user_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
                          const CK_CHAR *newPin, CK_ULONG newLen);
  CK_RV v_change_so_pin(const CK_CHAR *oldPin, CK_ULONG oldLen,
                        const CK_CHAR *newPin, CK_ULONG newLen);
  ...
  void v_restore_state(CK_BBOOL token_initialized, CK_BBOOL user_pin_set,
...
private:
  ...
  static CK_RV
  v_verify_pin_with_lockout(const SecureBuffer &stored, std::size_t stored_len,
                            const CK_CHAR *candidate, CK_ULONG candidate_len,
                            CK_BBOOL pin_set, CK_RV not_initialized_rv,
                            unsigned max_attempts,
                            std::atomic<unsigned> &counter,
                            std::atomic<CK_BBOOL> &locked, std::mutex &mutex);

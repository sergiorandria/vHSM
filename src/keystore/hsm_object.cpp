...
// (sensitive objects are non-copyable) but also want to catch bugs early. Throwing
// in copy construction (rather than silently failing or using a deleted copy ctor)
// alerts developers that they've attempted an invalid operation. Using a runtime
// check instead of = delete allows non-sensitive subclasses to enable copies 
// if needed without reimplementing the entire copy constructor.
...
HsmObject& HsmObject::operator=(const HsmObject& other) {
    ...
    // operation. We check before any mutations so that *this is unchanged on error.
    ...
    // overwrite the pointers. This is especially important if *this was temporary.
    wipe();
    ...
    , attrs_(std::move(other.attrs_))
{
    ...
    // in a "moved-from" state. We must leave it in a valid state (safe to destroy).
    // Resetting to default values ensures ~other doesn't try to wipe garbage data
    // or call virtual functions on a partially-destructed object.
...
void HsmObject::setAttribute(CK_ATTRIBUTE_TYPE type, const u8* data, std::size_t len) {
    ...
    // WHY assign not insert_or_assign: insert_or_assign returns a pair with "inserted"
    // flag, which we don't need. assign() is cleaner and updates existing or inserts new.

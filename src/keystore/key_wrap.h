...
namespace vhsm::keystore {
     ...
     *
     * WHY encapsulate RFC 3394: Key wrapping protects keys at rest. Unlike encryption
     * (which obscures plaintext), wrapping is designed specifically for key material:
     * it adds a 64-bit integrity check value (ICV) that detects tampering. If the
     * ICV doesn't match on unwrap, we know the key was corrupted or malicious.
     *
    ...
    public:
         ...
         * Accepting smaller would weaken security. Rejecting mismatches at construction
         * prevents accidental use of weak keys or misunderstanding the API contract.
         */
         ...
         * must be multiples of 8 to fit evenly into blocks. Minimum 16 bytes (2 blocks)
         * ensures the wrapped output is > 8 bytes, avoiding collision with empty keys.
         *
         ...
         *
         * WHY "6-round integrity matrix": RFC 3394 performs 6 rounds (3 forward, 3 backward)
         * of XOR and encryption. This is the standard, proven to be secure against 
         * known-plaintext attacks. The integrity check embedded in the ICV is verified
         * at the end—if it doesn't match, the key was tampered with and we REJECT.
         *
         * WHY throw on mismatch: A mismatched ICV is a cryptographic failure (attacker
         * tamper or corruption). We fail-closed: reject the key immediately rather than
         * returning a corrupted key to the caller.
         */

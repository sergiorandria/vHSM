...
// layer does raw input validation (null checks, length bounds) and delegates
// all business logic to v_TokenCore_M1. Separating concerns this way makes it
// easy to test the core in isolation and easier to port to other languages
// later.
//
// WHY Token is non-copyable: Each Token manages identity (label, id) and shared
// state (session counts). Copying would violate uniqueness constraints (two
// tokens with the same label could exist, breaking invariants). Non-copyable
// forces callers to use shared_ptr or references, preventing accidental
// duplication.
//
// WHY input validation happens here, not in the core: The core focuses on
// PKCS#11 semantics and state. Facade handles C API quirks (null pointers,
// length mismatches). This layering keeps the core clean and easier to reason
// about.
//
...
public:
  ...
  // at construction and are immutable. Passing them to the constructor makes
  // this constraint explicit and prevents partially-initialized tokens from
  // escaping.
  ...
  // WHY template methods here (not just core): create_object and
  // find_object_by_label_and_id are templates that return typed shared_ptr to
  // derived HsmObject classes (e.g., std::shared_ptr<PrivateKey>). Templates
  // can't be virtual, so we implement them as thin wrappers that forward to
  // core. This keeps the template instantiation in one place and maintains the
  // facade pattern.
  ...
  template <typename T>
  std::shared_ptr<T> find_object_by_label_and_id(const std::string &label,
                                                 const std::string &id) {
  ...
  CK_BBOOL is_user_pin_locked() const noexcept {
    return v_core_.v_is_user_pin_locked();
  }
  CK_BBOOL is_so_pin_locked() const noexcept {
    return v_core_.v_is_so_pin_locked();
  }
  unsigned user_pin_failed_attempts() const noexcept {
    return v_core_.v_user_failed_attempts();
  }
  unsigned so_pin_failed_attempts() const noexcept {
    return v_core_.v_so_failed_attempts();
  }
  void set_max_pin_attempts(unsigned max) {
    v_core_.v_set_max_failed_attempts(max);
  }
  unsigned max_pin_attempts() const noexcept {
    return v_core_.v_max_failed_attempts();
  }
  ...
  void restore_state(CK_BBOOL token_initialized, CK_BBOOL user_pin_set,
                     CK_BBOOL so_pin_set, CK_BBOOL user_login_required,
                     CK_BBOOL so_login_required, unsigned max_failed_attempts,
                     unsigned user_failed_attempts, unsigned so_failed_attempts,
                     CK_BBOOL user_pin_locked, CK_BBOOL so_pin_locked,
                     const std::vector<uint8_t> &kek);

  // WHY NOTE on getLoginState: In PKCS#11, login state is PER-SESSION, not
  // per-token. Different sessions can have different login states. The Token
  // only verifies PIN correctness. The Session object tracks who is logged in
  // (if anyone). This boundary prevents the Token from holding login state it
  // doesn't own, keeping concerns separate.

  // WHY get_kek() is accessible: The KeyWrap object needs the KEK to
  // wrap/unwrap keys. The core generates and stores the KEK; we expose it here
  // for the rest of the system. It's read-only (const method, const return).
  // The caller must not modify it.
  std::vector<std::uint8_t> get_kek() const;

private:
  // WHY SystemHsmClock here (not injected): Tests construct the core directly
  // with FrozenHsmClock for deterministic time. The public Token always uses
  // the system clock. This layering lets tests mock time while production uses
  // real time without cluttering the public interface with a clock parameter.
  vhsm::SystemHsmClock v_clock_;

  // WHY v_core_ is the real implementation: All state, locking, object
  // management, and PKCS#11 semantics live in the core. Token is just a thin
  // validation + delegation layer.

...
namespace vhsm::keystore {
    ...
    KeyWrap::KeyWrap(const std::vector<u8>& master_kek) {
        ...
        // a copy so we can apply security controls (mlock) without affecting their memory.
    ...
    KeyWrap::~KeyWrap() {
        ...
        if (!internal_kek.empty()) {
            ...
            // If we munlock first, the OS could swap the key to disk before cleanse runs.
    ...
    std::vector<u8> KeyWrap::wrap(const std::vector<u8>& plaintext_key) const {
        ...
        // could potentially recover both the wrapped key and the KEK from disk analysis.
        ...
        for (size_t j = 0; j <= 5; ++j) {
            ...
            for (size_t i = 1; i <= n; ++i) {
                ...
                // WHY memcpy back to result: Store the encrypted semi-block for the next round.
    ...
    std::vector<u8> KeyWrap::unwrap(const std::vector<u8>& ciphertext_key) const {
        ...
        // WHY n = (size/8) - 1: The ciphertext is (n+1) semi-blocks: 1 for ICV + n for data.
        // Subtract 1 to recover n (the number of data semi-blocks).
        size_t n = (ciphertext_key.size() / 8) - 1;
        
        // WHY allocate n*8 bytes: The unwrapped key is the data portion (without the ICV).
        ...
        uint8_t block[16];
        // WHY j counts backward (5 down to 0): RFC 3394 unwrap reverses the wrap process.
        // We undo the 6 rounds in reverse order.
        for (ssize_t j = 5; j >= 0; --j) {
            ...
            for (ssize_t i = n; i >= 1; --i) {
                ...
                // WHY call decrypt_block: We encrypt during wrap, so we decrypt during unwrap.
                ...
                // 8 bytes are the (incrementally updated) ICV; the last 8 are the original data.
                std::memcpy(&A, block, 8);
                std::memcpy(result.data() + ((i - 1) * 8), block + 8, 8);
            }
        }

        // WHY verify A == AIV: If unwrap succeeded, the ICV must match the expected value.
        // This is the integrity check. If A != AIV, the key was corrupted or decrypted
        // with the wrong KEK, and we REJECT (fail-closed).
        if (A != AIV) {

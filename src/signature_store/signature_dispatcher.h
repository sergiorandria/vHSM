...
// WHY SignatureDispatcher is a public facade: PKCS#11 signing flows through CryptoEngine,
// which returns a SignResult (signature bytes + mechanism + digest). SignatureDispatcher takes
// that result and orchestrates the persistence/audit/notification/ledger workflow:
...
//
// WHY signature store is complex: Blockchain integration is eventual-consistent. A signature
// is created locally (database), then asynchronously committed to the ledger. If the ledger
// fails, retry queues hold the entry until success. The dispatcher manages this state machine.
//
// WHY inject dependencies (clock, bus, audit_log, ledger_worker): Decouples signature storage
// from specific implementations. Production uses real database/ledger; tests can mock them.
// Dependency injection enables testing without blockchain or database setup.
//
// WHY dispatch method is the single orchestration point: All signatures flow through this
// method. Centralizes policy:
...
public:
    // WHY constructor takes multiple dependencies: All are required for dispatch to work.
    ...
    // - ledger_worker: optional ledger submitter (async blockchain commit, nullptr = skip ledger)
    SignatureDispatcher(
        IDbConnection& conn,
    ...
    // WHY dispatch is the single public method: Centralizes the entire signing workflow.
    // Takes a SignResult (from CryptoEngine) and contextual metadata, then orchestrates:
...
private:
    // WHY SystemHsmClock: Injected for testability. Records creation timestamps for audit.
    // Tests inject FrozenHsmClock; production uses system time.

...
SignatureDispatcher::SignatureDispatcher(
    ...
    : v_clock_(), v_core_(conn, token, notification_bus, audit_log,
                          ledger_worker, v_clock_) {}

bool SignatureDispatcher::dispatch(
    const vhsm::crypto::SignResult &sign_result, int64_t created_at,
    int slot_id, const std::string &token_label, const std::string &key_id,
    const std::string &key_fingerprint, const std::string &mechanism,
    const std::string &digest_algorithm, const std::string &session_handle,
  ...
  if (sign_result.signature.empty() || key_fingerprint.empty() ||
      mechanism.empty()) {
    return false;
  }

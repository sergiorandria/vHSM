...
std::string
NotificationDispatcher::event_type_name(const NotificationEvent &event) {
  switch (event.type) {
  case NotificationEvent::EventType::SIGN_CREATED:
    return "SIGN_CREATED";
  case NotificationEvent::EventType::DB_WRITE_FAILED:
    return "DB_WRITE_FAILED";
  case NotificationEvent::EventType::LEDGER_COMMIT_FAILED:
    return "LEDGER_COMMIT_FAILED";
  case NotificationEvent::EventType::LEDGER_COMMITTED:
    return "LEDGER_COMMITTED";
  case NotificationEvent::EventType::LEDGER_VERIFY_FAILED:
    return "LEDGER_VERIFY_FAILED";
  case NotificationEvent::EventType::VERIFY_COMPLETED:
    return "VERIFY_COMPLETED";
  case NotificationEvent::EventType::VERIFY_FAILED:
    return "VERIFY_FAILED";
  case NotificationEvent::EventType::ENCRYPT_COMPLETED:
    return "ENCRYPT_COMPLETED";
  case NotificationEvent::EventType::DECRYPT_COMPLETED:
    return "DECRYPT_COMPLETED";
  case NotificationEvent::EventType::WRAP_KEY_COMPLETED:
    return "WRAP_KEY_COMPLETED";
  case NotificationEvent::EventType::UNWRAP_KEY_COMPLETED:
    return "UNWRAP_KEY_COMPLETED";
  case NotificationEvent::EventType::KEY_ROTATED:
    return "KEY_ROTATED";
  case NotificationEvent::EventType::KEY_DESTROYED:
    return "KEY_DESTROYED";
  case NotificationEvent::EventType::INTEGRITY_ALERT:
    return "INTEGRITY_ALERT";
  case NotificationEvent::EventType::ADMIN_LOGIN:
    return "ADMIN_LOGIN";
  case NotificationEvent::EventType::PIN_LOCKOUT:
    return "PIN_LOCKOUT";
  }
  return "UNKNOWN";
}
...
bool NotificationDispatcher::matches(const NotificationSubscriber &sub,
  ...
  case NotificationEvent::Severity::CRITICAL:
    event_rank = 3;
    break;
  case NotificationEvent::Severity::WARNING:
    event_rank = 2;
    break;
  case NotificationEvent::Severity::INFO:
    event_rank = 1;
    break;
  }
...
bool NotificationDispatcher::deliver_with_retry(
    const NotificationSubscriber &sub, const NotificationEvent &event) {
  ...
  for (int attempt = 0; attempt <= retries; ++attempt) {
    ...
    repo_.log_notification(
        vhsm::utils::uuid_v4(), event.timestamp, event_type_name(event), sub.id,
        attempt < retries ? "RETRYING" : "FAILED", attempt + 1, last_error);
  }

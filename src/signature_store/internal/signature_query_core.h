...
public:
  ...
  v_get_signature_by_id(const std::string &signature_id) const;

  v_SignatureQueryVerificationResult_M1
  v_verify_signature(const std::string &signature_id) const;

  v_SignatureQueryVerificationResult_M1
  v_verify_signature(const std::string &signature_id,
                     vhsm::ledger::LedgerClient &ledger);

  std::vector<std::string>
  v_get_signature_ids_by_key_fingerprint(const std::string &key_fingerprint);

  std::vector<std::string> v_get_signature_ids_by_time_range(int64_t start_time,
                                                             int64_t end_time);

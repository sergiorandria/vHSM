...
v_VerificationServiceCore_M1::v_VerificationServiceCore_M1(
   ...
   : v_conn_(conn),
     v_ledger_client_(ledger_client),
     v_signature_repository_(signature_repository) {
}

v_VerificationResult_M1 v_VerificationServiceCore_M1::v_verify(
    const std::string& signature_id, bool check_ledger) {
    ...
    if (local_ledger_tx_id_opt) {
        auto ledger_entry_opt = v_ledger_client_.get_record(*local_ledger_tx_id_opt);
        if (ledger_entry_opt) {

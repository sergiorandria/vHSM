...
struct v_VerificationResult_M1 {
    ...
    bool payload_digest_match = false;     // Ledger payload digest matches local payload digest
    bool signature_b64_match = false;      // Ledger signature matches local signature
    bool key_fingerprint_match = false;    // Ledger key fingerprint matches local key fingerprint

...
v_SignatureQueryCore_M1::v_SignatureQueryCore_M1(IDbConnection& conn, vhsm::keystore::Token& token)
   : v_conn_(conn), v_signature_repository_(conn, token) {
}

std::optional<std::vector<std::optional<std::string>>> v_SignatureQueryCore_M1::v_get_signature_by_id(
    const std::string& signature_id) const {
    return v_signature_repository_.get_by_id(signature_id);
}

v_SignatureQueryVerificationResult_M1 v_SignatureQueryCore_M1::v_verify_signature(
    const std::string& signature_id) const {
...
v_SignatureQueryVerificationResult_M1 v_SignatureQueryCore_M1::v_verify_signature(
    ...
    v_SignatureQueryVerificationResult_M1 result = v_verify_signature(signature_id);
...
std::vector<std::string> v_SignatureQueryCore_M1::v_get_signature_ids_by_key_fingerprint(
...
std::vector<std::string> v_SignatureQueryCore_M1::v_get_signature_ids_by_time_range(
    int64_t start_time, int64_t end_time) {
    const std::string sql = R"SQL(
        SELECT id FROM signature_records WHERE created_at >= ? AND created_at <= ?;
    )SQL";
    try {
        auto rs = v_conn_.query(sql, {std::to_string(start_time), std::to_string(end_time)});
        std::vector<std::string> ids;
        ids.reserve(rs.rows_.size());
        for (const auto& row : rs.rows_) {
            if (row.column_count() > 0 && row.get_string(0)) {

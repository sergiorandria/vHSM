...
std::optional<std::vector<std::optional<std::string>>> SignatureQuery::get_signature_by_id(
...
SignatureQuery::VerificationResult SignatureQuery::verify_signature(
    const std::string& signature_id) const {
    // Public API: reject degenerate input before delegating to the core.
    if (signature_id.empty()) {
...
std::vector<std::string> SignatureQuery::get_signature_ids_by_time_range(

...
NotificationRepository::NotificationRepository(IDbConnection &conn)
    : conn_(conn) {}

bool NotificationRepository::add_subscriber(
    const std::string &id, const std::string &name, const std::string &channel,
    const std::string &address, const std::string &min_severity,
    const std::optional<std::string> &event_filter, bool enabled) {
...
bool NotificationRepository::update_subscriber(
    const std::string &id, const std::string &name, const std::string &channel,
    const std::string &address, const std::string &min_severity,
    const std::optional<std::string> &event_filter, bool enabled) {
  const std::string sql = R"SQL(
  ...
  try {
    conn_.exec(sql,
               {name, channel, address, min_severity, event_filter.value_or(""),
                std::to_string(enabled ? 1 : 0), id});
    return true;
  } catch (const DbError &e) {
    return false;
  }
...
NotificationRepository::get_subscriber(const std::string &id) const {
  const std::string sql = R"SQL(
    ...
    for (size_t i = 0; i < row.column_count(); ++i) {
      ...
      if (i == 6) { // enabled column is integer
        if (opt) {
          ...
          result.push_back(val != 0 ? std::make_optional<std::string>("true")
                                    : std::make_optional<std::string>("false"));
        } else {
...
NotificationRepository::get_all_subscriber_ids() const {
...
std::vector<std::string>
NotificationRepository::get_enabled_subscriber_ids() const {
  std::vector<std::string> ids;
...
NotificationRepository::get_enabled_subscribers() const {
  ...
  try {
    ...
    for (const auto &row : rs.rows_) {
      ...
      sub.event_filter =
          filter.empty() ? std::nullopt : std::optional<std::string>(filter);
      sub.enabled = true;
...
bool NotificationRepository::log_notification(
    const std::string &id, int64_t sent_at, const std::string &event_id,
    const std::string &subscriber_id, const std::string &outcome,
...
std::vector<std::string>
NotificationRepository::get_failed_notifications(int max_attempts) const {
  std::vector<std::string> ids;
...
bool NotificationRepository::update_notification_outcome(
    const std::string &id, const std::string &outcome, int attempt_count,
  ...
  try {
    conn_.exec(sql, {outcome, std::to_string(attempt_count),
                     error_detail.value_or(""), id});
    return true;
  } catch (const DbError &e) {
    return false;
  }

...
// The HMAC covers all columns of the row except the integrity_hmac column
// itself. The column order must match the order in the CREATE TABLE statement
// for signature_records.
class RowIntegrity {
public:
  ...
  std::string compute_hmac(
      const std::vector<std::optional<std::string>> &column_values) const;

  // Verifies that the given HMAC matches the computed HMAC for the column
  // values.

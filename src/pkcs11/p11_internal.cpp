...
std::unordered_map<CK_SESSION_HANDLE, std::vector<CK_OBJECT_HANDLE>> g_objectRegistry;
...
std::unordered_map<CK_SESSION_HANDLE, std::vector<CK_OBJECT_HANDLE>> g_findResults;
...
keystore::Slot* p11_get_slot(CK_SLOT_ID id) {
    auto sp = vhsm::session::SlotManager::get_instance().get_slot(static_cast<u64>(id));
    return sp.get();
}
...
CK_MECHANISM_TYPE class_to_mech(ObjectType t) {
    switch (t) {
        case ObjectType::PUBLIC_KEY:  return CKM_RSA_PKCS;
        case ObjectType::PRIVATE_KEY: return CKM_RSA_PKCS;
        case ObjectType::SECRET_KEY:  return CKM_AES_ECB;
...
vhsm::crypto::HashAlgorithm mech_to_hash(CK_MECHANISM_TYPE m) {
    switch (m) {
        case CKM_SHA256: case CKM_SHA256_RSA_PKCS:
        case CKM_ECDSA_SHA256: case CKM_SHA256_HMAC: return vhsm::crypto::HashAlgorithm::SHA256;
        case CKM_SHA384: case CKM_SHA384_RSA_PKCS:
        case CKM_ECDSA_SHA384: case CKM_SHA384_HMAC: return vhsm::crypto::HashAlgorithm::SHA384;
        case CKM_SHA512: case CKM_SHA512_RSA_PKCS:
        case CKM_ECDSA_SHA512: case CKM_SHA512_HMAC: return vhsm::crypto::HashAlgorithm::SHA512;
        default: return vhsm::crypto::HashAlgorithm::SHA256;
    }
}

bool is_rsa_mech(CK_MECHANISM_TYPE m) {
    return m == CKM_RSA_PKCS || m == CKM_RSA_X_509 || m == CKM_SHA256_RSA_PKCS ||
           m == CKM_SHA384_RSA_PKCS || m == CKM_SHA512_RSA_PKCS || m == CKM_RSA_PKCS_OAEP ||
           m == CKM_RSA_PKCS_PSS || m == CKM_SHA256_RSA_PKCS_PSS ||
           m == CKM_SHA384_RSA_PKCS_PSS || m == CKM_SHA512_RSA_PKCS_PSS;
}
bool is_ec_mech(CK_MECHANISM_TYPE m) {
    return m == CKM_ECDSA || m == CKM_ECDSA_SHA256 || m == CKM_ECDSA_SHA384 || m == CKM_ECDSA_SHA512;
}
...
CK_RV p11_apply_template(HsmObject& obj, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount) {
    for (CK_ULONG i = 0; i < ulCount; ++i) {
        ...
        if (a->pValue == nullptr && a->ulValueLen != 0) return CKR_ARGUMENTS_BAD;
        if (a->pValue == nullptr) {
            // deletion not directly supported on HsmObject; skip
            continue;
        }
        obj.setAttribute(a->type, static_cast<const u8*>(a->pValue), a->ulValueLen);
    }
    return CKR_OK;
}
...
CK_RV p11_store_key(HsmObject& obj, EVP_PKEY* pkey, bool isPrivate, int keyType) {
    ...
    if (base == EVP_PKEY_RSA) {
        ...
        RSA_free(rsa);
        obj.setAttribute(CKA_CLASS, reinterpret_cast<const u8*>(&g_cka_public_key), sizeof(CK_OBJECT_CLASS));
        (void)keyType;
    ...
    if (isPrivate)
        obj.setAttribute(CKA_CLASS, reinterpret_cast<const u8*>(&g_cka_private_key), sizeof(CK_OBJECT_CLASS));
    else
        obj.setAttribute(CKA_CLASS, reinterpret_cast<const u8*>(&g_cka_public_key), sizeof(CK_OBJECT_CLASS));
    CK_KEY_TYPE kt = (base == EVP_PKEY_RSA) ? CKK_RSA : CKK_EC;
    obj.setAttribute(CKA_KEY_TYPE, reinterpret_cast<const u8*>(&kt), sizeof(CK_KEY_TYPE));
    return CKR_OK;
}
...
const char* digest_name(vhsm::crypto::HashAlgorithm h) {
    switch (h) {
        case vhsm::crypto::HashAlgorithm::SHA256: return "SHA256";
        case vhsm::crypto::HashAlgorithm::SHA384: return "SHA384";
        case vhsm::crypto::HashAlgorithm::SHA512: return "SHA512";
...
CK_RV p11_rsa_encrypt(EVP_PKEY* key, const std::vector<u8>& in, std::vector<u8>& out,
    ...
    if (EVP_PKEY_CTX_set_rsa_padding(ctx, padding) <= 0) { EVP_PKEY_CTX_free(ctx); return CKR_GENERAL_ERROR; }
    if (padding == RSA_PKCS1_OAEP_PADDING) {
        const EVP_MD* omd = mgf1_md.empty() ? EVP_sha256()
                                            : EVP_get_digestbyname(mgf1_md.c_str());
        if (!omd) omd = EVP_sha256();
        EVP_PKEY_CTX_set_rsa_mgf1_md(ctx, omd);
        EVP_PKEY_CTX_set_rsa_oaep_md(ctx, omd);
    }
    if (label && !label->empty()) {
        u8* copy = static_cast<u8*>(OPENSSL_malloc(label->size() ? label->size() : 1));
        std::memcpy(copy, label->data(), label->size());
        EVP_PKEY_CTX_set0_rsa_oaep_label(ctx, copy, label->size());
    }
    size_t outlen = 0;
    if (EVP_PKEY_encrypt(ctx, nullptr, &outlen, in.data(), in.size()) <= 0) { EVP_PKEY_CTX_free(ctx); return CKR_GENERAL_ERROR; }
    out.resize(outlen);
    if (EVP_PKEY_encrypt(ctx, out.data(), &outlen, in.data(), in.size()) <= 0) { EVP_PKEY_CTX_free(ctx); return CKR_GENERAL_ERROR; }
    out.resize(outlen);
...
CK_RV p11_rsa_decrypt(EVP_PKEY* key, const std::vector<u8>& in, std::vector<u8>& out,
    ...
    size_t outlen = 0;
    if (EVP_PKEY_decrypt(ctx, nullptr, &outlen, in.data(), in.size()) <= 0) { EVP_PKEY_CTX_free(ctx); return CKR_GENERAL_ERROR; }
    out.resize(outlen);
    if (EVP_PKEY_decrypt(ctx, out.data(), &outlen, in.data(), in.size()) <= 0) { EVP_PKEY_CTX_free(ctx); return CKR_GENERAL_ERROR; }
    out.resize(outlen);
    EVP_PKEY_CTX_free(ctx);
    return CKR_OK;
}
...
CK_RV p11_rsa_sign(EVP_PKEY* key, const std::vector<u8>& digest, std::vector<u8>& sig,
    ...
    const EVP_MD* md = mdName.empty() ? nullptr : EVP_get_digestbyname(mdName.c_str());
    ...
    size_t siglen = 0;
    if (EVP_PKEY_sign(ctx, nullptr, &siglen, digest.data(), digest.size()) <= 0) { EVP_PKEY_CTX_free(ctx); return CKR_GENERAL_ERROR; }
    sig.resize(siglen);
    if (EVP_PKEY_sign(ctx, sig.data(), &siglen, digest.data(), digest.size()) <= 0) { EVP_PKEY_CTX_free(ctx); return CKR_GENERAL_ERROR; }
    sig.resize(siglen);
    EVP_PKEY_CTX_free(ctx);
    return CKR_OK;
}
...
CK_RV p11_rsa_verify(EVP_PKEY* key, const std::vector<u8>& digest, const std::vector<u8>& sig,
    ...
    int rc = EVP_PKEY_verify(ctx, sig.data(), sig.size(), digest.data(), digest.size());
    EVP_PKEY_CTX_free(ctx);
...
std::vector<u8> p11_ecdh_derive(EVP_PKEY* priv, EVP_PKEY* peer) {
    ...
    if (EVP_PKEY_derive_init(ctx) <= 0) { EVP_PKEY_CTX_free(ctx); return secret; }
    if (EVP_PKEY_derive_set_peer(ctx, peer) <= 0) { EVP_PKEY_CTX_free(ctx); return secret; }
    size_t len = 0;
    if (EVP_PKEY_derive(ctx, nullptr, &len) <= 0) { EVP_PKEY_CTX_free(ctx); return secret; }
    secret.resize(len);
    if (EVP_PKEY_derive(ctx, secret.data(), &len) <= 0) { EVP_PKEY_CTX_free(ctx); return {}; }
    secret.resize(len);
    EVP_PKEY_CTX_free(ctx);
    return secret;
}
...
CK_RV p11_aes_gcm_encrypt(const std::vector<u8>& key, const std::vector<u8>& iv,
    ...
    int rc = CKR_OK;
    do {
        if (EVP_EncryptInit_ex(ctx, cipher, nullptr, nullptr, nullptr) != 1) { rc = CKR_GENERAL_ERROR; break; }
        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, static_cast<int>(iv.size()), nullptr) != 1) { rc = CKR_GENERAL_ERROR; break; }
        if (EVP_EncryptInit_ex(ctx, nullptr, nullptr, key.data(), iv.data()) != 1) { rc = CKR_GENERAL_ERROR; break; }
        int len = 0;
        if (!aad.empty() && EVP_EncryptUpdate(ctx, nullptr, &len, aad.data(), static_cast<int>(aad.size())) != 1) { rc = CKR_GENERAL_ERROR; break; }
        ct.resize(pt.size());
        if (EVP_EncryptUpdate(ctx, ct.data(), &len, pt.data(), static_cast<int>(pt.size())) != 1) { rc = CKR_GENERAL_ERROR; break; }
        int finalLen = 0;
        if (EVP_EncryptFinal_ex(ctx, ct.data() + len, &finalLen) != 1) { rc = CKR_GENERAL_ERROR; break; }
        ct.resize(static_cast<size_t>(len) + static_cast<size_t>(finalLen));
        tag.resize(16);
        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, tag.data()) != 1) { rc = CKR_GENERAL_ERROR; break; }
    } while (0);
...
CK_RV p11_aes_gcm_decrypt(const std::vector<u8>& key, const std::vector<u8>& iv,
    ...
    do {
        ...
        if (!aad.empty() && EVP_DecryptUpdate(ctx, nullptr, &len, aad.data(), static_cast<int>(aad.size())) != 1) break;
        pt.resize(ct.size());
        if (EVP_DecryptUpdate(ctx, pt.data(), &len, ct.data(), static_cast<int>(ct.size())) != 1) break;
        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, static_cast<int>(tag.size()), const_cast<u8*>(tag.data())) != 1) break;
        int finalLen = 0;
...
EVP_PKEY* p11_build_key_from_attrs(HsmObject* obj, bool /*isPrivate*/, int /*keyType*/) {
    ...
    if ((rsaPriv && !rsaPriv->empty())) {
        ...
        RSA* rsa = d2i_RSAPrivateKey(nullptr, &p, static_cast<long>(rsaPriv->size()));
    ...
    if ((ecPriv && !ecPriv->empty())) {
        ...
        EC_KEY* ec = d2i_ECPrivateKey(nullptr, &p, static_cast<long>(ecPriv->size()));
    ...
    if ((ecPub && !ecPub->empty())) {
        ...
        if (params && point && !params->empty() && !point->empty()) {
            ...
            EC_GROUP* grp = d2i_ECPKParameters(nullptr, &pp, static_cast<long>(params->size()));
            if (grp) {
                EC_KEY* ec = EC_KEY_new();
                EC_KEY_set_group(ec, grp);
                EC_POINT* pt = EC_POINT_new(grp);
                if (EC_POINT_oct2point(grp, pt, point->data(), point->size(), nullptr) == 1) {

...
// Concrete implementations for NotificationBus and AuditLog (defined here for use in C_Initialize)
...
public:
    void append(const std::string& event_id, const std::string& event_type) override {
        // In production, this would write to an audit database/file
        // For now, log to stderr for debugging
        std::cerr << "[AUDIT] event_id=" << event_id << " event_type=" << event_type << std::endl;
    }
};
...
// WHY extern g_initialized: Shared across translation units in the PKCS#11 module.
// p11_init.cpp sets/clears it; p11_internal.cpp reads it via p11_is_initialized().
extern bool g_initialized;

// SignatureDispatcher and related globals
extern std::unique_ptr<vhsm::signature_store::db::SignatureDispatcher> g_signatureDispatcher;
...
extern std::unique_ptr<vhsm::signature_store::db::NotificationDispatcher> g_notificationDispatcher;
extern std::unique_ptr<vhsm::signature_store::db::NotificationRepository> g_notificationRepo;
...
// WHY CKA_VHSM_* constants: Internal vendor-defined attributes for storing key material.
// These are not standard PKCS#11 attributes; they store DER-encoded key blobs so the
// PKCS#11 layer can reconstruct EVP_PKEY objects from HSM objects.
...
std::shared_ptr<keystore::HsmObject> p11_get_object(CK_SESSION_HANDLE hSession, CK_OBJECT_HANDLE hObject);
...
// forbids using the same session concurrently from multiple threads, but different
// sessions ARE used concurrently. Guarding unordered_map structure (rehash) and each
// keyed value with one short-lived mutex is simpler and cheaper than per-map or
// per-session mutexes, and prevents concurrent C_* calls from two sessions racing
// on the shared containers. The lock is only held for map access, never across
// OT or crypto work, so it cannot deadlock with the object store / session manager.
extern std::mutex g_stateMutex;

// Reset all per-session crypto-operation state.
void p11_reset_op(CK_SESSION_HANDLE h);

// WHY extern "C": The PKCS#11 API functions (C_Initialize, C_GetSlotList, etc.) must have
// C linkage so they are not name-mangled.  The declarations below let each p11_*.cpp
// implementation file see C linkage when it defines the functions, ensuring the symbols
// exported from the .so match the extern "C" expectations in function_list.cpp.
extern "C" {
...
CK_RV C_DecryptUpdate(CK_SESSION_HANDLE hSession, CK_BYTE_PTR pPart, CK_ULONG ulPartLen, CK_BYTE_PTR pDecryptedPart, CK_ULONG_PTR pulDecryptedPartLen);

...
std::string resolve_db_path_for_container() {
  ...
  if (home && *home) {
    ...
    if (local && *local) base = std::filesystem::path(local) / "vHSM";
    else {
...
static std::unique_ptr<vhsm::persistence::Vault> open_or_create_vault() {
  ...
  if (!path_cstr || !*path_cstr || !pass_cstr) return nullptr;
  ...
  try {
    if (std::filesystem::exists(vault_path)) {
      ...
      if (v->is_valid()) vhsm::persistence::restore_token_from_vault(*token, *v);
      return v;
    } else {

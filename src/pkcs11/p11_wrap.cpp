...
bool raw_key_bytes(HsmObject* o, std::vector<u8>& out) {
    ...
    const auto* rp = o->findAttribute(CKA_VHSM_RSA_PRIV); if (rp && !rp->empty()) { out = *rp; return true; }
    const auto* ep = o->findAttribute(CKA_VHSM_EC_PRIV); if (ep && !ep->empty()) { out = *ep; return true; }
    const auto* ru = o->findAttribute(CKA_VHSM_RSA_PUB); if (ru && !ru->empty()) { out = *ru; return true; }
    const auto* eu = o->findAttribute(CKA_VHSM_EC_PUB); if (eu && !eu->empty()) { out = *eu; return true; }
    return false;
}
...
CK_RV create_from_raw(CK_SESSION_HANDLE hSession, const std::vector<u8>& raw,
    ...
    if (pk) {
        auto [h, ptr] = store.v_create_object<HsmObject>(ObjectType::PRIVATE_KEY, true, true, false, true);
        CK_RV rv = p11_apply_template(*ptr, pTemplate, ulCount);
        if (rv == CKR_OK) p11_store_key(*ptr, pk, true, EVP_PKEY_get_base_id(pk) == EVP_PKEY_RSA ? CKK_RSA : CKK_EC);
        EVP_PKEY_free(pk);
    ...
    if (raw.size() == 16 || raw.size() == 24 || raw.size() == 32) {
        auto [h, ptr] = store.v_create_object<HsmObject>(ObjectType::SECRET_KEY, false, true, false, false);
        CK_RV rv = p11_apply_template(*ptr, pTemplate, ulCount);
        if (rv == CKR_OK) p11_store_secret(*ptr, raw);
        if (rv != CKR_OK) { store.v_destroy_object(h); return rv; }
        *phKey = h;
        return CKR_OK;
    }
...
CK_RV C_WrapKey(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism,
    ...
    if (pMechanism->mechanism == CKM_AES_KEY_WRAP) {
        ...
        if (!kekAttr || kekAttr->size() != 32) return CKR_KEY_SIZE_RANGE;
        try {
            KeyWrap kw(*kekAttr);
            out = kw.wrap(raw);
...
CK_RV C_UnwrapKey(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism,
    ...
    if (pMechanism->mechanism == CKM_AES_KEY_WRAP) {
        ...
        try {
            KeyWrap kw(*kekAttr);
            raw = kw.unwrap(std::vector<u8>(pWrappedKey, pWrappedKey + ulWrappedKeyLen));
        } catch (...) { return CKR_WRAPPED_KEY_INVALID; }
    } else if (pMechanism->mechanism == CKM_RSA_PKCS ||
               pMechanism->mechanism == CKM_RSA_PKCS_OAEP) {
...
CK_RV C_DeriveKey(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism,
    ...
    EVP_PKEY* peer = d2i_PUBKEY(nullptr, &p, static_cast<long>(params->ulPublicDataLen));
    if (!peer) {
        EVP_PKEY_free(priv);
        return CKR_MECHANISM_PARAM_INVALID;
    }
    ...
    EVP_PKEY_free(priv);
    EVP_PKEY_free(peer);
    if (secret.empty()) return CKR_GENERAL_ERROR;

...
const std::vector<CK_MECHANISM_TYPE>& supported_mechanisms() {
    static const std::vector<CK_MECHANISM_TYPE> m = {
        ...
        CKM_AES_ECB, CKM_AES_CBC, CKM_AES_GCM, CKM_AES_KEY_GEN, CKM_AES_KEY_WRAP,
        CKM_ECDH1_DERIVE,
    };
    return m;
}
...
void fill_mech_info(CK_MECHANISM_TYPE t, CK_MECHANISM_INFO_PTR info) {
    ...
    if (t == CKM_RSA_PKCS || t == CKM_RSA_X_509 || t == CKM_RSA_PKCS_OAEP ||
        t == CKM_RSA_PKCS_PSS || t == CKM_SHA256_RSA_PKCS || t == CKM_SHA384_RSA_PKCS ||
        t == CKM_SHA512_RSA_PKCS || t == CKM_SHA256_RSA_PKCS_PSS ||
        t == CKM_SHA384_RSA_PKCS_PSS || t == CKM_SHA512_RSA_PKCS_PSS) {
        info->ulMinKeySize = 2048; info->ulMaxKeySize = 4096;
    } else if (t == CKM_EC_KEY_PAIR_GEN || t == CKM_ECDSA || t == CKM_ECDSA_SHA256 ||
               t == CKM_ECDSA_SHA384 || t == CKM_ECDSA_SHA512 || t == CKM_ECDH1_DERIVE) {
        ...
        info->ulMinKeySize = 128; info->ulMaxKeySize = 256;
    } else {
...
CK_RV C_GetSlotList(CK_BBOOL tokenPresent, CK_SLOT_ID_PTR pSlotList, CK_ULONG_PTR pulCount) {
    ...
    if (tokenPresent) {
        ...
        for (auto id : ids) {
            ...
            if (sp && sp->is_token_present()) filt.push_back(id);
        }
        ids = filt;
    }
    if (!pSlotList) { *pulCount = static_cast<CK_ULONG>(ids.size()); return CKR_OK; }
    if (*pulCount < ids.size()) {
        *pulCount = static_cast<CK_ULONG>(ids.size());
        return CKR_BUFFER_TOO_SMALL;
    }
    for (CK_ULONG i = 0; i < ids.size(); ++i) pSlotList[i] = static_cast<CK_SLOT_ID>(ids[i]);
    *pulCount = static_cast<CK_ULONG>(ids.size());
    return CKR_OK;
}
...
CK_RV C_GetSlotInfo(CK_SLOT_ID slotID, CK_SLOT_INFO_PTR pInfo) {
    ...
    if (!slot) return CKR_SLOT_ID_INVALID;
    std::memset(pInfo, 0, sizeof(CK_SLOT_INFO));
    copy_fixed(pInfo->slotDescription, sizeof(pInfo->slotDescription), slot->get_description().c_str());
    copy_fixed(pInfo->manufacturerID, sizeof(pInfo->manufacturerID), slot->get_manufacturer().c_str());
...
CK_RV C_GetTokenInfo(CK_SLOT_ID slotID, CK_TOKEN_INFO_PTR pInfo) {
    ...
    if (!tok) return CKR_SLOT_ID_INVALID;
    ...
    copy_fixed(pInfo->serialNumber, sizeof(pInfo->serialNumber), tok->get_id().c_str());
    CK_FLAGS flags = CKF_RNG;
    if (tok->is_token_initialized())     flags |= CKF_TOKEN_INITIALIZED;
    if (tok->is_user_login_required())   flags |= CKF_LOGIN_REQUIRED;
    if (tok->is_user_pin_set())          flags |= CKF_USER_PIN_INITIALIZED;
    if (tok->is_so_pin_set())            flags |= CKF_SO_PIN_INITIALIZED;
    pInfo->flags = flags;
    ...
    pInfo->ulMaxPinLen = 256; pInfo->ulMinPinLen = 4;
...
CK_RV C_GetMechanismList(CK_SLOT_ID slotID, CK_MECHANISM_TYPE_PTR pMechanismList, CK_ULONG_PTR pulCount) {
    ...
    for (CK_ULONG i = 0; i < m.size(); ++i) pMechanismList[i] = m[i];
    *pulCount = static_cast<CK_ULONG>(m.size());
    return CKR_OK;
}
...
CK_RV C_InitToken(CK_SLOT_ID slotID, CK_UTF8CHAR_PTR pPin, CK_ULONG ulPinLen, CK_UTF8CHAR_PTR pLabel) {
    ...
    if (!tok) return CKR_SLOT_ID_INVALID;
    if (pPin && ulPinLen > 0) {
        CK_RV rv = tok->initialize_so_pin(reinterpret_cast<const CK_CHAR*>(pPin), ulPinLen);
...
CK_RV C_InitPIN(CK_SESSION_HANDLE hSession, CK_UTF8CHAR_PTR pPin, CK_ULONG ulPinLen) {
    ...
    return tok->initialize_user_pin(reinterpret_cast<const CK_CHAR*>(pPin), ulPinLen);
}
...
CK_RV C_SetPIN(CK_SESSION_HANDLE hSession, CK_UTF8CHAR_PTR pOldPin, CK_ULONG ulOldLen,
    ...
    if (pOldPin && ulOldLen > 0)
        return tok->change_user_pin(reinterpret_cast<const CK_CHAR*>(pOldPin), ulOldLen,
                                     reinterpret_cast<const CK_CHAR*>(pNewPin), ulNewLen);
    return tok->set_user_pin(nullptr, 0, reinterpret_cast<const CK_CHAR*>(pNewPin), ulNewLen);
}

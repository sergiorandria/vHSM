...
//
// The core authenticates a role's PIN against a token and, on success,
// publishes an ADMIN_LOGIN (INFO) event on the notification bus (spec: "SO or
// USER authenticated via gRPC admin").  Failed attempts fall through to the
// token's PIN lockout counter and return CKR_PIN_* codes unchanged.

...
func (gc *GatewayClient) ExecuteTransaction(chaincodeFuncName string, args ...string) ([]byte, error) {
	proposal, err := gc.GetContract().NewProposal(
		chaincodeFuncName,
		client.WithArguments(args...),
	)
	if err != nil {
		return nil, fmt.Errorf("failed to create ledger proposal for %s: %w", chaincodeFuncName, err)
	}

	transaction, err := proposal.Endorse()
	if err != nil {
		return nil, fmt.Errorf("failed to endorse ledger proposal for %s: %w", chaincodeFuncName, err)
	}

	// Result() contient la valeur renvoyee par la fonction du chaincode
	// (ex: le thesisID genere par CreateThesis), disponible des l'endossement.
	result := transaction.Result()

	commit, err := transaction.Submit()
	if err != nil {
		return nil, fmt.Errorf("failed to submit transaction %s to ledger: %w", chaincodeFuncName, err)
	}

	status, err := commit.Status()
	if err != nil {
		return nil, fmt.Errorf("failed to obtain commit status for %s: %w", chaincodeFuncName, err)
	}
	if !status.Successful {
		return nil, fmt.Errorf("transaction %s committed with unsuccessful status: %v", chaincodeFuncName, status.Code)
	}

...
std::shared_ptr<GrpcConnection>
GrpcConnection::connect(const std::string &target, const TlsCredentials &tls,
  ...
  auto channel = ::grpc::CreateCustomChannel(
      target, creds, makeChannelArguments(tls, options));
  return std::shared_ptr<GrpcConnection>(new GrpcConnection(
      std::move(channel), target, options.waitForReadyTimeout));
}

std::shared_ptr<GrpcConnection>
GrpcConnection::connectInsecure(const std::string &target,
                                const ChannelOptions &options) {
  TlsCredentials none;
  auto channel =
      ::grpc::CreateCustomChannel(target, ::grpc::InsecureChannelCredentials(),

...
std::string randomNonce(size_t length) {
    ...
    if (!nonce.empty() && RAND_bytes(reinterpret_cast<unsigned char*>(nonce.data()),
...
msp::SerializedIdentity makeSerializedIdentity(const identity::Identity& identity) {
    msp::SerializedIdentity serialized;
    serialized.set_mspid(identity.getMSPID());
    serialized.set_id_bytes(crypto::X509Certificate(identity.getCertificate()).getDER());
    return serialized;
}
...
::protos::Proposal createProposal(const identity::Identity& identity,
                                  const std::string& channelId,
                                  const std::string& txId,
                                  const std::string& chaincodeName,
...
::protos::SignedProposal signProposal(const identity::Identity& identity,
    ...
    signedProposal.set_signature(signBytes(identity, signedProposal.proposal_bytes()));

...
public:
  ...
  std::string getPrivateKeyPEM() const {
    ...
    if (PEM_write_bio_ECPrivateKey(bio, ec_key_, nullptr, nullptr, 0, nullptr,
                                   nullptr) != 1) {
      BIO_free(bio);
  ...
  std::string sign(const std::string &data) const {
    ...
    if (ECDSA_sign(0, reinterpret_cast<const unsigned char *>(data.data()),
                   static_cast<int>(data.size()), sig, &sig_len,
                   ec_key_) != 1) {
      OPENSSL_free(sig);
  ...
  bool verify(const std::string &data, const std::string &signature) const {
    ...
    // Verify signature
    return ECDSA_verify(0, reinterpret_cast<const unsigned char *>(data.data()),
                        static_cast<int>(data.size()), sigBytes.data(),
                        static_cast<int>(sigBytes.size()), ec_key_) == 1;
  }
  ...
  std::string signDigest(const std::string &digest) const {
    ...
    ECDSA_SIG *sig =
        ECDSA_do_sign(reinterpret_cast<const unsigned char *>(digest.data()),
                      static_cast<int>(digest.size()), ec_key_);
    if (!sig) {
...
// Constructor
ECKeyPair::ECKeyPair(const std::string &pem)
    : pimpl_(std::make_unique<Impl>(pem)) {}
...
// Verification
bool ECKeyPair::verify(const std::string &data,

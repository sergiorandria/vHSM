...
std::string CSR::generate(const std::string &privateKeyPEM,
  ...
  if (!organization.empty() &&
      X509_NAME_add_entry_by_txt(
          name, "O", MBSTRING_ASC,
          reinterpret_cast<const unsigned char *>(organization.c_str()), -1, -1,
          0) != 1) {
    X509_NAME_free(name);
    X509_REQ_free(req);
    EVP_PKEY_free(pkey);
  ...
  if (!locality.empty() &&
      X509_NAME_add_entry_by_txt(
          name, "L", MBSTRING_ASC,
          reinterpret_cast<const unsigned char *>(locality.c_str()), -1, -1,
          0) != 1) {
    X509_NAME_free(name);
    X509_REQ_free(req);
    EVP_PKEY_free(pkey);
  ...
  if (!state.empty() &&
      X509_NAME_add_entry_by_txt(
          ...
          reinterpret_cast<const unsigned char *>(state.c_str()), -1, -1,
          0) != 1) {
    X509_NAME_free(name);
    X509_REQ_free(req);
    EVP_PKEY_free(pkey);
  ...
  if (!country.empty() &&
      X509_NAME_add_entry_by_txt(
          name, "C", MBSTRING_ASC,
          reinterpret_cast<const unsigned char *>(country.c_str()), -1, -1,
          0) != 1) {
    X509_NAME_free(name);
    X509_REQ_free(req);
    EVP_PKEY_free(pkey);
  ...
  if (!sans.empty()) {
    ...
    X509_EXTENSION *extItem = X509V3_EXT_conf_nid(
        nullptr, &ctx, OBJ_txt2nid("subjectAltName"), sanList.c_str());
    if (extItem) {

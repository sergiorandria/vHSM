...
public:
  explicit Impl(const std::string &certPEM) : cert_(nullptr) {
    BIO *bio =
        BIO_new_mem_buf(certPEM.data(), static_cast<int>(certPEM.size()));
    if (!bio) {
  ...
  std::vector<std::string> getSubjectAlternativeNames() const {
    ...
    while ((loc = X509_get_ext_by_NID(cert_, NID_subject_alt_name, loc)) >= 0) {
      ...
      if (!extName || std::string(extName) != "subjectAltName")
        continue;
      ...
      for (int i = 0; i < sanCount; ++i) {
        ...
        if (san->type == GEN_DNS) {
          ...
          if (len > 0 && dns) {
            sans.push_back(std::string(reinterpret_cast<char *>(dns), len));
            OPENSSL_free(dns);
          }
        }
      }
...
private:
  ...
  asn1TimeToSystemPoint(const ASN1_TIME *time) const {
    if (!time) {
      return std::chrono::system_clock::time_point();
    }
...
// Constructor
X509Certificate::X509Certificate(const std::string &certPEM)
    : pimpl_(std::make_unique<Impl>(certPEM)) {}

X509Certificate::~X509Certificate() = default;

// Getters
std::string X509Certificate::getPEM() const { return pimpl_->getPEM(); }

std::string X509Certificate::getSubjectCommonName() const {
  return pimpl_->getSubjectCommonName();
}

std::string X509Certificate::getIssuer() const { return pimpl_->getIssuer(); }
...
bool X509Certificate::isValid() const { return pimpl_->isValid(); }

std::string X509Certificate::getPublicKeyPEM() const {
  return pimpl_->getPublicKeyPEM();
}

std::string X509Certificate::getDER() const { return pimpl_->getDER(); }

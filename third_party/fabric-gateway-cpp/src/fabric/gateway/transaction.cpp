...
std::string txValidationCodeName(int32_t code) {
  ...
  case ::protos::VALID:
    return "VALID";
  case ::protos::NIL_ENVELOPE:
    return "NIL_ENVELOPE";
  case ::protos::BAD_PAYLOAD:
    return "BAD_PAYLOAD";
  case ::protos::BAD_COMMON_HEADER:
    return "BAD_COMMON_HEADER";
  case ::protos::BAD_CREATOR_SIGNATURE:
    return "BAD_CREATOR_SIGNATURE";
  case ::protos::INVALID_ENDORSER_TRANSACTION:
    return "INVALID_ENDORSER_TRANSACTION";
  case ::protos::INVALID_CONFIG_TRANSACTION:
    return "INVALID_CONFIG_TRANSACTION";
  case ::protos::UNSUPPORTED_TX_PAYLOAD:
    return "UNSUPPORTED_TX_PAYLOAD";
  case ::protos::BAD_PROPOSAL_TXID:
    return "BAD_PROPOSAL_TXID";
  case ::protos::DUPLICATE_TXID:
    return "DUPLICATE_TXID";
  case ::protos::ENDORSEMENT_POLICY_FAILURE:
    return "ENDORSEMENT_POLICY_FAILURE";
  case ::protos::MVCC_READ_CONFLICT:
    return "MVCC_READ_CONFLICT";
  case ::protos::PHANTOM_READ_CONFLICT:
    return "PHANTOM_READ_CONFLICT";
  case ::protos::UNKNOWN_TX_TYPE:
    return "UNKNOWN_TX_TYPE";
  case ::protos::TARGET_CHAIN_NOT_FOUND:
    return "TARGET_CHAIN_NOT_FOUND";
  case ::protos::MARSHAL_TX_ERROR:
    return "MARSHAL_TX_ERROR";
  case ::protos::NIL_TXACTION:
    return "NIL_TXACTION";
  case ::protos::EXPIRED_CHAINCODE:
    return "EXPIRED_CHAINCODE";
  case ::protos::CHAINCODE_VERSION_CONFLICT:
    return "CHAINCODE_VERSION_CONFLICT";
  case ::protos::BAD_HEADER_EXTENSION:
    return "BAD_HEADER_EXTENSION";
  case ::protos::BAD_CHANNEL_HEADER:
    return "BAD_CHANNEL_HEADER";
  case ::protos::BAD_RESPONSE_PAYLOAD:
    return "BAD_RESPONSE_PAYLOAD";
  case ::protos::BAD_RWSET:
    return "BAD_RWSET";
  case ::protos::ILLEGAL_WRITESET:
    return "ILLEGAL_WRITESET";
  case ::protos::INVALID_WRITESET:
    return "INVALID_WRITESET";
  case ::protos::INVALID_CHAINCODE:
    return "INVALID_CHAINCODE";
  case ::protos::NOT_VALIDATED:
    return "NOT_VALIDATED";
  case ::protos::INVALID_OTHER_REASON:
...
void throwOnError(const ::grpc::Status &status, const std::string &what) {
  if (!status.ok()) {
    throw fabric::grpc::StatusException(status.error_code(),
                                        status.error_message(), what);
  }
}

} // namespace

Transaction::Transaction(std::shared_ptr<Contract> contract, std::string name,
                         std::map<std::string, std::string> transient)
    : contract_(std::move(contract)), name_(std::move(name)),
      transient_(std::move(transient)) {}

TransactionResult Transaction::submit() { return submit({}); }
...
TransactionResult Transaction::submit(const std::vector<std::string> &args) {
  ...
  *endorseRequest.mutable_proposed_transaction() = signedProposal;
  ...
  if (prepared.payload().empty()) {
    throw fabric::grpc::StatusException(
        ::grpc::StatusCode::INTERNAL,
        "gateway returned an empty prepared transaction", "Endorse");
  }
  ...
  *submitRequest.mutable_prepared_transaction() = prepared;
  ...
  signedCommitRequest.set_signature(
      protoutil::signBytes(identity, requestBytes));
  ...
  ::protos::Response chaincodeResponse =
      protoutil::extractProposalResponse(prepared);

...
std::shared_ptr<Contract>
Network::getContract(const std::string &chaincodeName) {
  return std::shared_ptr<Contract>(
      new Contract(shared_from_this(), chaincodeName));
}

std::shared_ptr<Gateway> Network::gateway() { return gateway_; }

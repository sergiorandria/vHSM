...
TEST(GrpcStatusTest, StatusCodeName_AllCodes) {
  ...
  EXPECT_STREQ(fabric::grpc::statusCodeName(grpc::StatusCode::NOT_FOUND),
               "NOT_FOUND");
  EXPECT_STREQ(fabric::grpc::statusCodeName(grpc::StatusCode::UNAVAILABLE),
               "UNAVAILABLE");
  EXPECT_STREQ(
      fabric::grpc::statusCodeName(grpc::StatusCode::PERMISSION_DENIED),
      "PERMISSION_DENIED");
}
...
TEST(GrpcStatusTest, StatusException_CarriesCodeAndMessage) {
  StatusException exc(grpc::StatusCode::NOT_FOUND, "no such chaincode",
                      "extra detail");
...
TEST_F(InsecureConnectionTest, ServerRejectionSurfacesAsStatusException) {
  ...
  EXPECT_TRUE(std::string(status.error_message()).find("injected failure") !=
              std::string::npos);

  // The SDK maps non-OK statuses to StatusException.
  StatusException exc(status.error_code(), status.error_message(),
...
protected:
  void startTlsServer(const CertFixture &fixture) {
    ...
    sslOpts.pem_key_cert_pairs.push_back(
        grpc::SslServerCredentialsOptions::PemKeyCertPair{fixture.leaf.key,
                                                          fixture.leaf.cert});
    port_ = freePort();
    ASSERT_GT(port_, 0);
    serverBuilder_.AddListeningPort(targetFor(port_),
...
TEST_F(TlsConnectionTest, TrustedCaRoundTrip) {
  ...
  auto conn =
      GrpcConnection::connect(targetFor(port_), creds, testOptions(5000));
  conn->waitForReady();
  EXPECT_TRUE(conn->isReady());
...
TEST_F(TlsConnectionTest, UntrustedCaIsRejected) {
  ...
  auto conn =
      GrpcConnection::connect(targetFor(port_), creds, testOptions(2000));

...
public:
    explicit FakeGatewayService(const std::string& pubKeyPem) : pubKeyPem_(pubKeyPem) {}
    ...
    grpc::Status Evaluate(grpc::ServerContext*,
        ...
        if (args.empty()) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "empty chaincode args");
        }
    ...
    grpc::Status Endorse(grpc::ServerContext*,
        ...
        if (!proposal.ParseFromString(request->proposed_transaction().proposal_bytes())) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "bad proposal bytes");
        }

        ::protos::ChaincodeProposalPayload proposalPayload;
        if (!proposalPayload.ParseFromString(proposal.payload())) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "bad proposal payload");
        }
        ...
        *chaincodeAction.mutable_response() = result;
        ...
        endorsedAction.set_proposal_response_payload(chaincodeAction.SerializeAsString());
        ...
        *transaction.add_actions() = transactionAction;
        ...
        if (!payload.mutable_header()->ParseFromString(proposal.header())) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "bad proposal header");
        }
    ...
    grpc::Status Submit(grpc::ServerContext*,
        ...
        if (envelope.payload().empty()) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "empty envelope payload");
        }
        if (!verifyDerSignature(pubKeyPem_, envelope.payload(), envelope.signature())) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT,
    ...
    grpc::Status CommitStatus(grpc::ServerContext*,
                              const ::gateway::SignedCommitStatusRequest* request,
                              ::gateway::CommitStatusResponse* response) override {
        ++commitCount;
        if (!verifyDerSignature(pubKeyPem_, request->request(), request->signature())) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT,
        ...
        if (!statusRequest.ParseFromString(request->request())) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "bad commit status request");
        }
        ...
        if (statusRequest.identity().empty()) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "missing commit identity");
        }

        response->set_result(static_cast<::protos::TxValidationCode>(validationCode));
        response->set_block_number(blockNumber);
        return grpc::Status::OK;
    }

    grpc::Status ChaincodeEvents(grpc::ServerContext*, const ::gateway::SignedChaincodeEventsRequest*,
...
private:
    grpc::Status verifySignedProposal(const ::protos::SignedProposal& signedProposal) const {
        if (signedProposal.proposal_bytes().empty()) {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "empty proposal bytes");
        }
    ...
    static std::vector<std::string> proposalArgs(const ::protos::SignedProposal& signedProposal) {
        ::protos::Proposal proposal;
        if (!proposal.ParseFromString(signedProposal.proposal_bytes())) return {};

        ::protos::ChaincodeProposalPayload proposalPayload;
        if (!proposalPayload.ParseFromString(proposal.payload())) return {};

        ::protos::ChaincodeInvocationSpec invocation;
        if (!invocation.ParseFromString(proposalPayload.input())) return {};
...
protected:
    void SetUp() override {
        ...
        connection_ = GrpcConnection::connectInsecure(targetFor(port_), testOptions());
...
TEST_F(GatewayClientTest, EvaluateTransactionReturnsChaincodeResult) {
    TransactionResult result = contract_->evaluateTransaction("GetAsset", {"asset1"});
...
TEST_F(GatewayClientTest, SubmitTransactionEndorsesSignsAndCommits) {
    TransactionResult result = contract_->submitTransaction("CreateAsset", {"a1", "100"});
...
TEST_F(GatewayClientTest, TransactionHandleWithTransientData) {
    ...
    auto txn = contract_->createTransaction("CreatePrivateAsset", {{"secret", "opaque-value"}});
...
TEST_F(GatewayClientTest, RpcFailureThrowsStatusException) {
    ...
    EXPECT_THROW(contract_->evaluateTransaction("Missing", {"x"}), StatusException);
    try {
...
TEST_F(GatewayClientTest, EvaluateAndSubmitUseConsistentTransactionId) {
    TransactionResult eval = contract_->evaluateTransaction("GetAsset", {"asset2"});
    EXPECT_EQ(eval.payload, "echo:asset2");

    std::string evaluatedTxId = fake_->lastTxId;
    TransactionResult submit = contract_->submitTransaction("CreateAsset", {"a2", "50"});

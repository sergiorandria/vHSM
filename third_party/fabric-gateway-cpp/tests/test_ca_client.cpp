// test_ca_client.cpp — Phase 2 unit + integration tests for
// fabric::ca::CaClient.
//
...
//   docker run -p 7054:7054 hyperledger/fabric-ca:1.5 fabric-ca-server start -b
//   admin:adminpw)
...
std::string base64Decode(const std::string &in) {
  ...
  for (int &c : table)
    c = -1;
  for (int i = 0; i < 64; ++i)
    table[static_cast<unsigned char>(tbl[i])] = i;
  std::string out;
  int buffer = 0, bits = 0;
  for (char ch : in) {
    if (ch == '=' || ch == '\n' || ch == '\r' || ch == ' ')
...
Identity testIdentity(const std::string &cn) {
  ...
  BIO *kbio = BIO_new_mem_buf(privateKeyPEM.data(),
                              static_cast<int>(privateKeyPEM.size()));
  EC_KEY *key = PEM_read_bio_ECPrivateKey(kbio, nullptr, nullptr, nullptr);
  BIO_free(kbio);
  if (!key)
    throw std::runtime_error("testIdentity: failed to load test key");
...
bool verifyToken(const Identity &registrar, const std::string &method,
  ...
  if (authHeader.rfind(b64cert + ".", 0) != 0)
    return false;
  ...
  const std::string pubPEM =
      ECKeyPair(registrar.getPrivateKey()).getPublicKeyPEM();
  ...
  int ok =
      ECDSA_verify(0, reinterpret_cast<const unsigned char *>(digest.data()),
                   static_cast<int>(digest.size()),
                   reinterpret_cast<const unsigned char *>(sig.data()),
                   static_cast<int>(sig.size()), pubkey);
  EC_KEY_free(pubkey);
  return ok == 1;
}
...
struct FakeHttpClient : public HttpClient {
  ...
  request(HttpMethod method, const std::string &url,
          ...
          const std::optional<std::string> &body, int timeoutSeconds) override {
    (void)timeoutSeconds;
...
std::string headerValue(const FakeHttpClient::Call &call,
...
TEST(CaClientUnitTest, Enroll_SendsCsrAndDecodesCert) {
  auto http =
      std::make_shared<FakeHttpClient>(HttpResponse{201, enrollResponse(), {}});
  ...
  EXPECT_EQ(headerValue(call, "Authorization"),
            "Basic " + b64("admin:adminpw"));
...
TEST(CaClientUnitTest, Enroll_ServerErrorThrowsWithMessage) {
  std::string body =
      R"({"errors":[{"code":20,"message":"Authentication failure"}],"success":false})";
...
TEST(CaClientUnitTest, Reenroll_ReusesKeyAndUsesTokenAuth) {
  ...
  EXPECT_TRUE(
      verifyToken(registrar, "POST", "/api/v1/reenroll", *call.body, auth));
...
TEST(CaClientUnitTest, Register_PostsIdAndParsesSecret) {
  ...
  auto resp = client.registerIdentity(registrar, "u1", "user", std::nullopt,
                                      std::nullopt, std::nullopt, "org1");
  ...
  EXPECT_TRUE(
      verifyToken(registrar, "POST", "/api/v1/register", *call.body, auth));
...
TEST(CaClientUnitTest, Revoke_ParsesCrlAndRevokedCerts) {
  ...
  const std::string auth = headerValue(call, "Authorization");
  EXPECT_TRUE(
      verifyToken(registrar, "POST", "/api/v1/revoke", *call.body, auth));

  EXPECT_TRUE(contains(resp.crl, "BEGIN X509 CRL"));
}

TEST(CaClientUnitTest, GetCertificates_AuthenticatesWithToken) {
  std::string body = R"({"result":{"certs":[{"PEM":")" + b64(kFakeCertPEM) +
                     R"("}]},"success":true})";
  auto http = std::make_shared<FakeHttpClient>(HttpResponse{200, body, {}});
  CaClient client(http, "http://ca:7054");
  ...
  Identity registrar = testIdentity("admin");
  auto certs =
      client.getCertificates(registrar, "akanid123", std::nullopt, "awkid456");
  ...
  EXPECT_EQ(call.url,
            "http://ca:7054/api/v1/"
            "certificates?aki=akanid123&authority_key_identifier=awkid456");

  // GET body is empty and the token signs the exact query URI.
  const std::string auth = headerValue(call, "Authorization");
  EXPECT_TRUE(verifyToken(
      registrar, "GET",
...
protected:
  ...
  void SetUp() override {
    ...
    client_ = std::make_unique<CaClient>(http_, "http://localhost:7054",
                                         std::nullopt, "Org1MSP");
  }
...
TEST_F(CaClientIntegrationTest, RegisterEnrollRevoke_RoundTrip) {
  ...
  auto reg = client_->registerIdentity(admin, user, "user", std::nullopt,
                                       std::nullopt, std::nullopt, "org1");

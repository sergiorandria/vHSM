#include "vhsm/scrypto/aes.h"
#include <cstring>

namespace vhsm::scrypto {
namespace {
static const uint8_t sbox[256] = {
    };
static const uint8_t rsbox[256] = {
    };
inline uint8_t xtime(uint8_t x) { return (x << 1) ^ ((x >> 7) & 1 ? 0x1b : 0); }
inline uint32_t pack(const uint8_t b[4]) {
  return (uint32_t(b[0]) << 24) | (uint32_t(b[1]) << 16) |
         (uint32_t(b[2]) << 8) | b[3];
}
inline void unpack(uint32_t v, uint8_t b[4]) {
  b[0] = v >> 24;
  b[1] = v >> 16;
  b[2] = v >> 8;
  b[3] = v;
}

static const uint32_t Rcon[15] = {0,    0x01, 0x02, 0x04, 0x08,
                                  0x10, 0x20, 0x40, 0x80, 0x1b,
                                  0x36, 0x6c, 0xd8, 0xab, 0x4d};

} // namespace

Aes256Key Aes256Key::expand(const uint8_t key[32]) {
  Aes256Key k{};
  uint32_t w[60];
  for (int i = 0; i < 8; i++)
    w[i] = pack(key + i * 4);
  for (int i = 8; i < 60; i++) {
    uint32_t temp = w[i - 1];
    if (i % 8 == 0) {
      // RotWord + SubWord + Rcon
      uint8_t b[4];
      unpack(temp, b);
      uint8_t t = b[0];
      b[0] = b[1];
      b[1] = b[2];
      b[2] = b[3];
      b[3] = t;
      for (int j = 0; j < 4; j++)
        b[j] = sbox[b[j]];
      temp = pack(b) ^ (Rcon[i / 8] << 24);
    } else if (i % 8 == 4) {
      uint8_t b[4];
      unpack(temp, b);
      for (int j = 0; j < 4; j++)
        b[j] = sbox[b[j]];
      temp = pack(b);
    }
    w[i] = w[i - 8] ^ temp;
  }
  for (int i = 0; i < 60; i++)
    k.rk[i] = w[i];
  return k;
}

namespace {
inline void add_round_key(uint8_t s[16], const uint32_t *rk) {
  for (int i = 0; i < 4; i++) {
    uint8_t b[4];
    unpack(rk[i], b);
    s[i * 4] ^= b[0];
    s[i * 4 + 1] ^= b[1];
    s[i * 4 + 2] ^= b[2];
    s[i * 4 + 3] ^= b[3];
  }
}
inline void sub_bytes(uint8_t s[16]) {
  for (int i = 0; i < 16; i++)
    s[i] = sbox[s[i]];
}
inline void inv_sub_bytes(uint8_t s[16]) {
  for (int i = 0; i < 16; i++)
    s[i] = rsbox[s[i]];
}
inline void shift_rows(uint8_t s[16]) {
  uint8_t t[16];
  std::memcpy(t, s, 16);
  s[1] = t[5];
  s[5] = t[9];
  s[9] = t[13];
  s[13] = t[1];
  s[2] = t[10];
  s[6] = t[14];
  s[10] = t[2];
  s[14] = t[6];
  s[3] = t[15];
  s[7] = t[3];
  s[11] = t[7];
  s[15] = t[11];
}
inline void inv_shift_rows(uint8_t s[16]) {
  uint8_t t[16];
  std::memcpy(t, s, 16);
  s[1] = t[13];
  s[5] = t[1];
  s[9] = t[5];
  s[13] = t[9];
  s[2] = t[10];
  s[6] = t[14];
  s[10] = t[2];
  s[14] = t[6];
  s[3] = t[7];
  s[7] = t[11];
  s[11] = t[15];
  s[15] = t[3];
}
inline void mix_cols(uint8_t s[16]) {
  for (int i = 0; i < 4; i++) {
    uint8_t a0 = s[i * 4], a1 = s[i * 4 + 1], a2 = s[i * 4 + 2],
            a3 = s[i * 4 + 3];
    uint8_t t = a0 ^ a1 ^ a2 ^ a3;
    uint8_t u = a0;
    s[i * 4] ^= t ^ xtime(a0 ^ a1);
    s[i * 4 + 1] ^= t ^ xtime(a1 ^ a2);
    s[i * 4 + 2] ^= t ^ xtime(a2 ^ a3);
    s[i * 4 + 3] ^= t ^ xtime(a3 ^ u);
  }
}
inline uint8_t mul(uint8_t a, uint8_t b) {
  uint8_t p = 0;
  for (int i = 0; i < 8; i++) {
    if (b & 1)
      p ^= a;
    bool hi = a & 0x80;
    a <<= 1;
    if (hi)
      a ^= 0x1b;
    b >>= 1;
  }
  return p;
}
inline void inv_mix_cols(uint8_t s[16]) {
  for (int i = 0; i < 4; i++) {
    uint8_t a0 = s[i * 4], a1 = s[i * 4 + 1], a2 = s[i * 4 + 2],
            a3 = s[i * 4 + 3];
    s[i * 4] = mul(a0, 0x0e) ^ mul(a1, 0x0b) ^ mul(a2, 0x0d) ^ mul(a3, 0x09);
    s[i * 4 + 1] =
        mul(a0, 0x09) ^ mul(a1, 0x0e) ^ mul(a2, 0x0b) ^ mul(a3, 0x0d);
    s[i * 4 + 2] =
        mul(a0, 0x0d) ^ mul(a1, 0x09) ^ mul(a2, 0x0e) ^ mul(a3, 0x0b);
    s[i * 4 + 3] =
        mul(a0, 0x0b) ^ mul(a1, 0x0d) ^ mul(a2, 0x09) ^ mul(a3, 0x0e);
  }
}
} // namespace

void aes256_encrypt_block(const Aes256Key &rk, const uint8_t in[16],
                          uint8_t out[16]) {
  uint8_t s[16];
  std::memcpy(s, in, 16);
  add_round_key(s, &rk.rk[0]);
  for (int r = 1; r < 14; r++) {
    sub_bytes(s);
    shift_rows(s);
    mix_cols(s);
    add_round_key(s, &rk.rk[r * 4]);
  }
  sub_bytes(s);
  shift_rows(s);
  add_round_key(s, &rk.rk[56]);
  std::memcpy(out, s, 16);
}
void aes256_decrypt_block(const Aes256Key &rk, const uint8_t in[16],
                          uint8_t out[16]) {
  uint8_t s[16];
  std::memcpy(s, in, 16);
  add_round_key(s, &rk.rk[56]);
  for (int r = 13; r >= 1; --r) {
    inv_shift_rows(s);
    inv_sub_bytes(s);
    add_round_key(s, &rk.rk[r * 4]);
    inv_mix_cols(s);
  }
  inv_shift_rows(s);
  inv_sub_bytes(s);
  add_round_key(s, &rk.rk[0]);
  std::memcpy(out, s, 16);
}

void aes256_ecb_encrypt_block(const uint8_t key[32], const uint8_t in[16],
                              uint8_t out[16]) {
  auto rk = Aes256Key::expand(key);
  aes256_encrypt_block(rk, in, out);
}
void aes256_ecb_decrypt_block(const uint8_t key[32], const uint8_t in[16],
                              uint8_t out[16]) {
  auto rk = Aes256Key::expand(key);
  aes256_decrypt_block(rk, in, out);
}

} // namespace vhsm::scrypto

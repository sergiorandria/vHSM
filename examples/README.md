...
## Fabric Gateway (`examples/*.cpp`)

The original Fabric Gateway examples (ported from `fabric-gateway-cpp`):

| File | What it shows |
|---|---|
| `hello_gateway.cpp` | Minimal submit + query via `Gateway` |
| `low_level_gateway.cpp` | Manual proposal / transaction build |
| `wallet_gateway.cpp` / `wallet_store.cpp` | File-system wallet + identity |
...
| # | File | Concepts |
|---|---|---|
| 01 | `01_init_and_login.cpp` | Library lifecycle, slots, sessions, PIN |
| 02 | `02_generate_keys.cpp` | `C_GenerateKeyPair` for RSA-2048, P-256, P-384 |
| 03 | `03_sign_verify.cpp` | `C_Sign`/`C_Verify` (ECDSA-SHA256 + RSA-SHA256), tamper check |
| 04 | `04_encrypt_decrypt.cpp` | `C_Encrypt`/`C_Decrypt` — AES-GCM + RSA-OAEP(SHA-256) |
| 05 | `05_digest_and_find.cpp` | `C_Digest` (SHA-256/384) + `C_FindObjects`/`C_GetAttributeValue` |
| 06 | `06_wrap_unwrap.cpp` | `C_WrapKey`/`C_UnwrapKey` (AES wrapped by RSA-OAEP) |
...
cmake --build build -j
./build/examples/pkcs11/01_init_and_login
./build/examples/pkcs11/03_sign_verify
# ...

...
# # =================================================================
# # SCRIPT ENTIÈREMENT AUTOMATISÉ : PACKAGING + DÉPLOIEMENT CCaaS + INSTALL
# # =================================================================
# # Détecte les canaux créés par generate-network.sh (network.env), puis pour
# # le chaincode choisi, AUTOMATIQUEMENT, pour chaque peer concerné :
# #
# #   1. Copie le code source Go fourni par l'utilisateur.
# #   2. Génère un Dockerfile (si absent) et CONSTRUIT l'image (docker build).
# #   3. Génère connection.json/metadata.json avec TLS dédié (cc-peer<N>),
# #      empaquette en .tar.gz.
# #   4. Injecte un service "cc-peer<N>.<domain>" dans docker-compose.yaml
# #      principal (sans rien casser dans le reste du fichier).
# #   5. Démarre ce service (docker-compose up -d).
# #   6. Attend que le port TCP du serveur chaincode réponde (healthcheck actif).
# #   7. Installe le package sur le peer correspondant via son conteneur CLI.
# #
# # Prérequis :
# #   1. ./generate-network.sh déjà exécuté (avec les hostnames cc-peer<N>
# #      générés dans crypto-config.yaml).
# #   2. docker-compose up -d déjà lancé pour le réseau (peers/orderer/CLI).
# #   3. Chaque organisation concernée a répondu "oui" à la question CLI.
# # =================================================================

...
#     IDENTITIES="orgadmin:orgadminpw:admin;user1:user1pw:client"
#     for ((p=0; p<${!PEERS_VAR}; p++)); do
#         IDENTITIES="${IDENTITIES};peer${p}:peer${p}pw:peer"
#     done

#     generate_ca_and_ldap_config "${!NAME_VAR}" "${!NAME_VAR}" "${!CA_PORT_VAR}" "${!OP_PORT_VAR}" \
#         "" "${!DB_NAME_VAR}" "${!DB_USER_VAR}" "${!DB_PASS_VAR}" \
#         "${!LDAP_PASS_VAR}" "$ORG_LDAP_DN" "$IDENTITIES"
# done
...
EOF
        ...
        # -------------------------------------------------------------
        # Identités applicatives LDAP pour ${O_NAME}, saisies interactivement.
        # Les rôles (businessCategory dans le LDIF) doivent correspondre
        # EXACTEMENT aux constantes RoleAdmin/RoleProfessor/RoleStudent du
        # package Go "internal" (admin / professeurs / etudiants), sinon
        # HasPermission() ne retrouvera jamais le rôle de l'utilisateur.
        # -------------------------------------------------------------
        echo ""
        echo "    --- Identités LDAP applicatives pour ${O_NAME} (rôle: admin) ---"
        read -p "    Identifiant de l'administrateur ${O_NAME} [orgadmin]: " O_ADMIN_UID
        O_ADMIN_UID=${O_ADMIN_UID:-orgadmin}
        read -p "    Mot de passe de l'administrateur ${O_NAME} [orgadminpw]: " O_ADMIN_PASS
        O_ADMIN_PASS=${O_ADMIN_PASS:-orgadminpw}

        while true; do
            read -p "    Combien de comptes 'professeurs' voulez-vous créer ? [1]: " O_NUM_PROF
            O_NUM_PROF=${O_NUM_PROF:-1}
            [[ "$O_NUM_PROF" =~ ^[0-9]+$ ]] && break
            echo "    Nombre invalide."
        done

        while true; do
            read -p "    Combien de comptes 'etudiants' voulez-vous créer ? [1]: " O_NUM_STUD
            O_NUM_STUD=${O_NUM_STUD:-1}
            [[ "$O_NUM_STUD" =~ ^[0-9]+$ ]] && break
            echo "    Nombre invalide."
        done

        cat << EOF >> $ENV_FILE
ORG${i}_ADMIN_UID="$O_ADMIN_UID"
ORG${i}_ADMIN_PASS="$O_ADMIN_PASS"
ORG${i}_NUM_PROFESSORS=$O_NUM_PROF
ORG${i}_NUM_STUDENTS=$O_NUM_STUD
EOF

        for ((j=0; j<O_NUM_PROF; j++)); do
            echo ""
            read -p "        Identifiant du professeur #$j [prof${j}]: " O_PROF_UID
            O_PROF_UID=${O_PROF_UID:-prof${j}}
            read -p "        Mot de passe du professeur #$j [prof${j}pw]: " O_PROF_PASS
            O_PROF_PASS=${O_PROF_PASS:-prof${j}pw}
            cat << PROF_EOF >> $ENV_FILE
ORG${i}_PROF${j}_UID="$O_PROF_UID"
ORG${i}_PROF${j}_PASS="$O_PROF_PASS"
PROF_EOF
        done

        for ((k=0; k<O_NUM_STUD; k++)); do
            echo ""
            read -p "        Identifiant de l'étudiant #$k [etud${k}]: " O_STUD_UID
            O_STUD_UID=${O_STUD_UID:-etud${k}}
            read -p "        Mot de passe de l'étudiant #$k [etud${k}pw]: " O_STUD_PASS
            O_STUD_PASS=${O_STUD_PASS:-etud${k}pw}
            cat << STUD_EOF >> $ENV_FILE
ORG${i}_STUD${k}_UID="$O_STUD_UID"
ORG${i}_STUD${k}_PASS="$O_STUD_PASS"
STUD_EOF
        done
...
for ((i=1; i<=NUM_ORGS; i++)); do
    ...
    # ---------------------------------------------------------------
    # Rôles applicatifs (businessCategory du LDIF) alignés sur les
    # constantes du package Go "internal" : RoleAdmin="admin",
    # RoleProfessor="professeurs", RoleStudent="etudiants".
    # Les identités "peer" restent des identités d'infrastructure Fabric
    # (NodeOU peer) et n'entrent pas dans le RBAC applicatif.
    # ---------------------------------------------------------------
    ADMIN_UID_VAR="ORG${i}_ADMIN_UID"; ADMIN_PASS_VAR="ORG${i}_ADMIN_PASS"
    NUM_PROF_VAR="ORG${i}_NUM_PROFESSORS"; NUM_STUD_VAR="ORG${i}_NUM_STUDENTS"

    IDENTITIES="${!ADMIN_UID_VAR}:${!ADMIN_PASS_VAR}:admin"

    for ((j=0; j<${!NUM_PROF_VAR}; j++)); do
        PROF_UID_VAR="ORG${i}_PROF${j}_UID"; PROF_PASS_VAR="ORG${i}_PROF${j}_PASS"
        IDENTITIES="${IDENTITIES};${!PROF_UID_VAR}:${!PROF_PASS_VAR}:professeurs"
    done

    for ((k=0; k<${!NUM_STUD_VAR}; k++)); do
        STUD_UID_VAR="ORG${i}_STUD${k}_UID"; STUD_PASS_VAR="ORG${i}_STUD${k}_PASS"
        IDENTITIES="${IDENTITIES};${!STUD_UID_VAR}:${!STUD_PASS_VAR}:etudiants"
    done

...
DOCKER_EOF
    ...
    for org_idx in ${!CH_ORGS_VAR}; do
        ...
        for ((p=0; p<${!PEERS_VAR}; p++)); do
            ...
            # --- Ask TLS preference FIRST — the TLS cert directory below is
            #     only actually needed if TLS is enabled, so we don't want to
            #     force its presence for people who plan to say 'n'. ---
            while true; do
            ...
            # NOTE: depuis la migration cryptogen -> Fabric-CA, le dossier
            # racine des identités s'appelle ${ORG_ROOT_DIR} (ex: ./organizations),
            # plus ${CRYPTO_CONFIG_DIR} (qui n'existe plus dans network.env).
            # Par ailleurs enroll-network.sh n'enrôle que des identités
            # "peer0", "peer1", ... (jamais "cc-peer0"), donc le certificat TLS
            # à utiliser pour le serveur CCaaS est celui déjà émis pour le
            # peer lui-même (${PEER_HOST}), pas un certificat "${CC_HOST}"
            # dédié qui n'a jamais été généré.
            CC_TLS_DIR="${ORG_ROOT_DIR}/peerOrganizations/${ORG_DOMAIN}/peers/${PEER_HOST}/tls"
            if [ "$TLS_ENABLED" == "true" ] && [ ! -d "$CC_TLS_DIR" ]; then
                echo "    ❌ Certificat TLS du peer introuvable : ${CC_TLS_DIR}"
                echo "       Relancez ./enroll-network.sh pour (re)générer ${ORG_ROOT_DIR}/."
                continue
            fi
